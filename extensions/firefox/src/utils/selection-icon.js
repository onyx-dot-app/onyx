(function () {
  const OPEN_SIDEBAR_WITH_INPUT = "openSidebarWithInput";
  ...
  function createSelectionIcon() {
    ...
    img.src = browser.runtime.getURL("public/icon32.png");
  ...
  function handleIconClick(e) {
    ...
    if (textToSend) {
      browser.runtime
        .sendMessage({
          action: OPEN_SIDEBAR_WITH_INPUT,
          selectedText: textToSend,
          pageUrl: window.location.href,
        })
        .catch((error) => {
          console.error("[Onyx] Error sending message:", error);
        });
    }
