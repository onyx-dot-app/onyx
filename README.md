...
GlomiAI 是基于 Onyx MIT 核心能力继续演进的中文消费级超级 Agent。Phase A 已完成中文对话、搜索、深度研究和平台模型能力的核心验证；当前进入 Phase B，重点是建设 **Glomi Forge** 自研生成交付运行时。
...
- **当前阶段**：Phase B Glomi Forge 王牌能力启动。
- **北极星体验**：用户不配置工具、不懂提示词，只输入中文需求，系统自动搜索、研究、分析、写作和交付。
- **工程策略**：硬 fork，自管后端，不依赖上游 Onyx 同步。
- **底座选择**：最大化复用 Onyx 的成熟 Agent runtime、Deep Research、工具系统和 Next.js 前端；Phase B 以 Glomi Forge（`glomi_forge`，Daytona + Pi）作为生成交付运行时，Onyx Craft/opencode 路径只作为参考与兜底。
...
| 模块 | 状态 | 已完成内容 | 下一步 |
|---|---|---|---|
| E1 i18n + 品牌替换 | Phase A 已验证 | 中文优先与 UI rebrand 的设计/计划已沉淀，复用现有 Web 前端做消费级重塑 | Phase B 继续服务 Glomi Forge 入口、分享页和生成物展示 |
| E2 平台模型目录 | Phase A 已验证，继续服务 Glomi Forge | 从单默认 LLM 演进为平台模型目录；后端同步模型能力画像，前端只暴露平台可选模型，不暴露 API key/base URL | Glomi Forge 侧复用同一模型目录，并处理 OpenAI-compatible 到 Pi/沙箱 builder 的映射 |
| E3 中文超级对话 | Phase A 已验证，转为主控入口 | 普通 chat tool guidance 引入中文搜索策略；`web_search` 支持 `lite / medium / deep`；Agent 在工具调用中自行选择搜索强度 | 增加“生成交付意图”识别，把页面/PPT/看板/小工具任务派发给 Glomi Forge |
| E4 中文深度研究 | Phase A 已验证，继续增强 | Deep Research research agent 默认 `mode=deep`；planner/orchestrator/research-agent/report prompt 已加入中文研究方法论 | 作为 Glomi Forge 的上游材料来源，为页面、报告、看板等生成物提供证据和结构 |
...
| Glomi Forge | Phase B 当前重心 | 已明确以 `glomi_forge` 自研运行时承接生成交付能力，Daytona 做 sandbox substrate，Pi 做 sandbox 内 builder harness | 先做 A 节点端到端、资源治理、模型适配，再做消费级模板入口、分享页和主控派发 |
| 超级编排层 E13 | Phase B 当前重心，依赖 Glomi Forge 稳定产物 | 复用 `CodingAgentTool` 与 `dr_loop` 原语，主对话仍是一号入口 | 在 Glomi Forge 可稳定生成后做自动路由、多子 agent、MoA 汇总 |
...
目标：用户注册后默认能用平台模型，不进入 Admin，不看到 API key/base URL；Phase B 中 Glomi Forge 也复用同一套模型目录。
...
平台 env
  ...
  -> chat / deep research / title / Glomi Forge provider mapping
```
...
- Glomi Forge 沙箱侧需要把 OpenAI-compatible 模型映射到 Pi/沙箱 builder 可用 provider，同时保留 GlomiAI 后端内部 provider 边界。
...
### 5. Glomi Forge 生成交付运行时

目标：把 **Glomi Forge** 做成 GlomiAI 的王牌交付能力。短问答和研究仍走 chat / Deep Research；需要页面、PPT、报告、看板、小工具、可分享成品时才进入 Forge。Onyx Craft/opencode 工作台仅作为参考实现与过渡兜底。
...
User Chinese request
  ...
  -> generate artifact: ForgeSession
...
2. **Glomi Forge A 节点端到端**：`glomi_forge` 地基 + Daytona sandbox + Pi builder + landing page template。
3. **生成物分享闭环**：让 Forge 输出默认可以公开/私密分享，分享页隐藏工作台复杂性。
4. **主对话派发 Glomi Forge**：主控识别生成交付意图，创建或复用 ForgeSession，把进度、预览和最终产物回填到对话。

保留/重建的 Forge 资产：

- sandbox 隔离和实时预览
- Pi builder harness / launcher 事件归一化
- 文件系统、上传、快照和恢复
...
- **Glomi Forge A 节点**：打通 `glomi_forge` 地基、ForgeSession/ForgeSpec/ForgeEvent、Daytona + Pi 落地页端到端。
- **Glomi Forge 运行稳定性**：把资源治理作为第一优先级，解决 sandbox 启动、卡死、回收、日志和失败恢复。
- **Glomi Forge 模型适配**：让 Forge 复用平台模型目录，验证 OpenAI-compatible 模型在 Pi builder、streaming、长上下文下的兼容性。
- **Glomi Forge C 端化**：沙箱镜像/网络国内化、消费级生成入口、模板化体验。
...
Phase B 先把 Glomi Forge 作为生成交付运行时打磨出来。鉴权、支付、合规、国内云部署、连接器生态和正式商业化会在 Phase C 推进；公网前必须先做内容安全和合规 gate。
...
| Phase B | Glomi Forge、生成物分享、超级编排层 | 当前重心 |
...
```text
web/
  Next.js 前端，消费级体验改造和聊天 / 研究 / Glomi Forge 入口

backend/onyx/
  chat/                 普通对话 agent loop
  deep_research/        深度研究多 agent 工作流
  tools/                web_search、open_url、python、Glomi Forge 等工具系统
  glomi_forge/          自研 Daytona + Pi 生成交付运行时（规划/实施中）
