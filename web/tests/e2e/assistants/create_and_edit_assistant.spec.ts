...
test.describe("Assistant Creation and Edit Verification", () => {
  // Configure this file to run serially to prevent race conditions between the tests.
  test.describe.configure({ mode: "serial" });

  let ccPairId: number;
  let documentSetId: number;
  let onyxApiClient: OnyxApiClient;
  ...
  test("should create assistant with user files when no connectors exist", async ({
    ...
    const assistantDescription = "Testing user file uploads without connectors";
  ...
  test("should create and edit assistant with Knowledge enabled", async ({
    ...
    onyxApiClient = new OnyxApiClient(page);
    ccPairId = await onyxApiClient.createFileConnector("Test Connector");
    documentSetId = await onyxApiClient.createDocumentSet("Test Document Set", [
      ccPairId,
    ]);
  ...
  test.afterAll(async ({ page }) => {
    // Always cleanup the connector and document set.
    if (ccPairId && documentSetId) {
      await page.context().clearCookies();
      await loginAs(page, "admin");
      const onyxApiClient = new OnyxApiClient(page);
      await onyxApiClient.deleteDocumentSet(documentSetId);
      await onyxApiClient.deleteCCPair(ccPairId);
    }
  });
});
