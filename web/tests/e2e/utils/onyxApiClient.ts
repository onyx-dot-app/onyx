...
export class OnyxApiClient {
  ...
  async createFileConnector(
    ...
    const responseData = await this.handleResponse<{ ccPairId: number }>(
      response,
      "Failed to create connector"
    );

    this.log(
      `Created file connector: ${connectorName} (CC Pair ID: ${responseData.ccPairId})`
    );

    return responseData.ccPairId;
  }
  ...
  async createDocumentSet(
    ...
    this.log(
      `Created document set: ${documentSetName} (ID: ${responseData.id})`
    );
