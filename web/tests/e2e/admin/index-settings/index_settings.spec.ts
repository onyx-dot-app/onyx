...
test.describe("Index Settings — contextual LLM updates @exclusive", () => {
  ...
  test("applies a contextual LLM only to new and updated documents", async ({
    ...
    await page
      ...
      .last()
      .fill(modelDisplayName(nextContextualModel));
    await page
      .getByText(modelDisplayName(nextContextualModel), { exact: true })
      .last()
      .click();
    ...
    await expect(
      page.getByRole("button", { name: "Rebuild all existing documents" })
    ).toBeVisible();
    await page
      .getByRole("button", {
        name: "Apply to new and updated documents",
