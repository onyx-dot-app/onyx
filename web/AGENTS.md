...
- **Update every locale when you touch a key.** `en.json` is the source of
  ...
  translation of the English value, following the register and pinned terms
  in `web/scripts/i18n/glossary.json`. This is blocking: `bun run i18n:check`
  (pre-commit) and `web/src/i18n/__tests__/catalog.test.ts` (CI) fail on
  missing keys, ICU-shape drift (arguments, tags, plurals), or orphan keys.
  The weekly `i18n-qa` workflow then reviews new and changed translations
  with an LLM (`bun run i18n:qa`) and opens a corrections PR; it owns
  `src/i18n/catalog.meta.json` — never hand-edit that file.
