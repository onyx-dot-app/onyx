...
- Type coverage (the share of identifiers whose type is not `any`, tests excluded) must not drop
  below the floors in `web/.type-coverage-baseline.yaml`. CI runs
  `ods type-coverage typescript --check`. After you remove `any` types, raise the floors with
