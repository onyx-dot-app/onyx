...
describe("billing actions", () => {
  ...
  describe("claimLicense (self-hosted only)", () => {
    test("calls license claim endpoint with session_id", async () => {
      // Mock POST /api/license/claim?session_id=xxx
      fetchSpy.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ success: true, message: "License claimed" }),
      } as Response);

      const result = await claimLicense("cs_test_abc123");

      expect(fetchSpy).toHaveBeenCalledWith(
        "/api/license/claim?session_id=cs_test_abc123",
        { method: "POST" }
      );

      expect(result).toEqual({ success: true, message: "License claimed" });
    });

    test("throws error on failed response", async () => {
      // Mock POST /api/license/claim (error)
      fetchSpy.mockResolvedValueOnce({
        ok: false,
        json: async () => ({ detail: "License not found" }),
      } as Response);

      await expect(claimLicense("invalid_session")).rejects.toThrow(
        "License not found"
      );
    });
  });
...
describe("billing actions (cloud mode)", () => {
  ...
  test("claimLicense throws error in cloud mode", async () => {
    // Re-import with cloud mode
    const { claimLicense: cloudClaimLicense } = await import("./actions");

    await expect(cloudClaimLicense("test_session")).rejects.toThrow(
