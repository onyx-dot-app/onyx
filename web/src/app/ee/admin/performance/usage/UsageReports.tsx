...
function GenerateReportInput({
  ...
  const requestReport = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw Error(`收到错误：${res.statusText}`);
      }
  ...
  return (
    <div className="mb-8">
      <Title className="mb-2">生成使用报告</Title>
      <Text as="p">生成工作区用户的使用统计。</Text>
      <Spacer rem={2} />
      ...
      >
        {isWaitingForReport ? "正在生成..." : "生成报告"}
      </OpalButton>
      <p className="mt-1 text-xs">
        {isWaitingForReport
          ? "报告正在生成中，请稍候..."
          : "报告会在后台生成。准备好后，可在下方“历史报告”区域下载。"}
      </p>
      {errorOccurred && (
        <ErrorCallout
          errorTitle="出现问题。"
...
function UsageReportsTable({
  ...
  return (
    <div>
      <Title className="mb-2 mt-6 mx-auto">历史报告</Title>
            ...
            <TableBody>
              {paginatedReports.map((r) => (
                <TableRow key={r.report_name}>
                  ...
                  <TableCell>{r.requestor ?? "自动生成"}</TableCell>
                  <TableCell>
...
export default function UsageReports() {
  ...
  const handleReportGenerated = () => {
    ...
    timeoutRef.current = setTimeout(() => {
      ...
      setTimeoutMessage(
        "报告生成时间比预期更长。报告会继续在后台生成，请几分钟后再查看。"
      );
