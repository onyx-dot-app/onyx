...
function GenerateReportInput({
  ...
  return (
    <div className="mb-8">
      <Title className="mb-2">{t("admin.usage.generate_reports_title")}</Title>
      <Text as="p">{t("admin.usage.generate_reports_desc")}</Text>
      <Spacer rem={2} />
          ...
          <Popover.Trigger asChild>
            ...
            >
              {dateRange?.from ? (
                ...
                <span>{t("admin.usage.pick_date_range")}</span>
              )}
            </Button>
          </Popover.Trigger>
      ...
      >
        {isWaitingForReport ? t("admin.usage.generating") : t("admin.usage.generate_report_btn")}
      </OpalButton>
      <p className="mt-1 text-xs">
        {isWaitingForReport
          ? t("admin.usage.generating_wait")
          : t("admin.usage.bg_generation_desc")}
      </p>
      {errorOccurred && (
        <ErrorCallout
          errorTitle={t("admin.usage.error_something_wrong")}
...
function UsageReportsTable({
  ...
  return (
    <div>
      <Title className="mb-2 mt-6 mx-auto">{t("admin.usage.previous_reports_title")}</Title>
      {usageReportsIsLoading && !isWaitingForReport ? (
        ...
        <>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>{t("admin.usage.table_report_col")}</TableHead>
                <TableHead>{t("admin.usage.table_period_col")}</TableHead>
                <TableHead>{t("admin.usage.table_generated_by_col")}</TableHead>
                <TableHead>{t("admin.usage.table_time_col")}</TableHead>
                <TableHead>{t("admin.usage.table_download_col")}</TableHead>
              </TableRow>
            </TableHeader>
                ...
                <TableRow key={r.report_name}>
                  ...
                  <TableCell>
                    {r.period_from
                      ...
                      : t("admin.usage.period_all_time")}
                  </TableCell>
                  <TableCell>{r.requestor ?? t("admin.usage.auto_generated")}</TableCell>
                  <TableCell>
              ...
              <div className="flex-1">
                <div className="text-status-warning-05">
                  <Text as="p" font="main-ui-action">
                    {t("admin.usage.report_in_progress_title")}
                  </Text>
                </div>
                <Spacer rem={0.25} />
                <div className="text-status-warning-05">
                  <Text as="p">{t("admin.usage.report_timeout_msg")}</Text>
                </div>
              </div>
            </div>
          </div>
