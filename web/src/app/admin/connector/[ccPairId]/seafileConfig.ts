...
interface SeafileValidationMessages {
  baseUrlRequired: string;
  baseUrlProtocol: string;
  libraryRequired: string;
  extensionRequired: string;
  extensionUnsupported: string;
  maxFileSizeInteger: string;
  maxFileSizePositive: string;
  maxFileSizeRequired: string;
}

export function createSeafileConnectorConfigSchema(
  messages: SeafileValidationMessages
): Yup.ObjectSchema<SeafileConnectorConfig> {
  return Yup.object({
    base_url: Yup.string()
      .trim()
      .required(messages.baseUrlRequired)
      .matches(/^https?:\/\//, messages.baseUrlProtocol),
    repo_ids: Yup.array()
      .of(Yup.string().required())
      .test(
        "has-repo-id",
        messages.libraryRequired,
    ...
    allowed_extensions: Yup.array()
      ...
      .test(
        "has-extension",
        messages.extensionRequired,
        (value) => uniqueNonEmpty(value).length > 0
      )
      .test("supported-extensions", messages.extensionUnsupported, (value) =>
    ...
    max_file_size_bytes: Yup.number()
      .integer(messages.maxFileSizeInteger)
      .positive(messages.maxFileSizePositive)
      .required(messages.maxFileSizeRequired),
  });
}

export const SeafileConnectorConfigSchema = createSeafileConnectorConfigSchema({
  baseUrlRequired: "Base URL is required",
  baseUrlProtocol: "Base URL must start with http:// or https://",
  libraryRequired: "At least one library ID is required",
  extensionRequired: "At least one file extension is required",
  extensionUnsupported:
    "One or more extensions are not supported by the Seafile connector",
  maxFileSizeInteger: "Max file size must be an integer",
  maxFileSizePositive: "Max file size must be greater than zero",
  maxFileSizeRequired: "Max file size is required",
});
