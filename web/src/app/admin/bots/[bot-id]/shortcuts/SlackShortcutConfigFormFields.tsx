...
export interface SlackShortcutConfigFormFieldsProps {
  isUpdate: boolean;
  isDefault: boolean;
...
export function SlackShortcutConfigFormFields({
  isUpdate,
  isDefault,
  documentSets,
  ...
  slack_bot_id,
  formikProps,
}: SlackShortcutConfigFormFieldsProps) {
      ...
      <div className="w-full">
        {isDefault && (
          <>
            ...
            <p className="mt-2 text-sm text-neutral-600">
              This default configuration will apply across all Slack shortcuts
              the bot is added to in the Slack workspace, unless disabled.
            </p>
            <div className="mt-4 p-4 bg-neutral-100 rounded-md border border-neutral-300">
              ...
              <p className="mt-2 text-sm text-neutral-600 italic">
                ...
                won&apos;t respond to shortcuts unless explicitly configured.
              </p>
            </div>
          </>
        )}
        {!isDefault && (
          <>
            ...
            />
            <label
              htmlFor="default_message"
              className="block text-text font-medium text-base mt-4 mb-2"
            >
              Default Message:
            </label>
            <Field name="default_message">
              {({ field, form }: { field: any; form: any }) => (
                <Textarea
                  {...field}
                  placeholder="Enter default message for this shortcut"
                  className="w-full min-h-24"
                />
              )}
            </Field>
            <p className="mt-2 text-sm dark:text-neutral-400 text-neutral-600">
              This message will be used as the default prompt when the shortcut is triggered.
            </p>
          </>
        )}
                        ...
                        <div className="mb-3 mt-2 flex gap-2 flex-wrap text-sm">
                          {unselectableSets.map((documentSet) => (
                            <DocumentSetSelectable
                              ...
                              documentSet={documentSet}
                              disabled
                              disabledTooltip="Unable to use this document set because it contains a connector with auto-sync permissions. OnyxBot's responses in this shortcut are visible to all Slack users, so mirroring the asker's permissions could inadvertently expose private information."
                              isSelected={false}
                              onSelect={() => {}}
                            />
                          ))}
