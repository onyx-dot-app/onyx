...
export interface ModelPickerPopoverProps {
  ...
  /** Pass the agent/persona id to scope the provider list. */
  personaId?: number;
...
export default function ModelPickerPopover({
  ...
  const { user } = useUser();
  const { llmProviders, isLoading } = useLLMProviders(personaId);
  ...
  return (
    ...
    >
      <div data-testid="model-picker-trigger">
        <Popover.Trigger asChild disabled={disabled}>
          {foldable ? (
            <OpenButton foldable icon={SvgRefreshCw} disabled={disabled}>
