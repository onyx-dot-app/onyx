...
// Default padding from the top where the anchor should appear (fallback if no sticky header)
const DEFAULT_ANCHOR_TOP_PADDING_PX = 64;
...
const DynamicBottomSpacer = React.memo(
  ({ anchorNodeId }: DynamicBottomSpacerProps) => {
    ...
    useEffect(() => {
      ...
      // When streaming ends, clean up smooth scroll flag
      if (streamingEnded) {
        const scrollContainer = getScrollContainer();
        if (scrollContainer) {
          scrollContainer.dataset.smoothScrollActive = "false";
        }
        // ResizeObserver will handle deactivation via debounce
      }
