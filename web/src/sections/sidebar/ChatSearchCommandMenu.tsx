...
function DynamicFooter() {
  const t = useTranslations("appShell.search");
  ...
  const actionLabel =
    highlightedItemType === "filter" ? t("showAll") : t("open");

  return (
    <CommandMenu.Footer
      leftActions={
        <>
          <CommandMenu.FooterAction icon={SvgArrowUpDown} label={t("select")} />
...
export default function ChatSearchCommandMenu({
  ...
  return (
    <>
      <div aria-label={t("openAriaLabel")} onClick={() => setOpen(true)}>
        {trigger}
      </div>
      ...
      <CommandMenu open={open} onOpenChange={handleOpenChange}>
        <CommandMenu.Content>
          ...
          <CommandMenu.List
            emptyMessage={
              hasSearchValue ? t("noResultsFound") : t("noChatsOrProjects")
            }
          >
