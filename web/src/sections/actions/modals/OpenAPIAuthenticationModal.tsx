...
type AuthMethod = "oauth" | "custom-header";
...
const validationSchema = Yup.object({
  ...
  headers: Yup.array()
    ...
    .when("authMethod", {
      is: "custom-header",
      then: (schema) => schema.min(1, "Add at least one authentication header"),
...
export default function OpenAPIAuthenticationModal({
  ...
  const initialValues: OpenAPIAuthFormValues = {
    ...defaultValues,
    authMethod: defaultMethod,
    headers: defaultValues.headers.map((header) => ({ ...header })),
  };
                    ...
                    <>
                      ...
                      <div className="flex flex-col gap-3 rounded-12 bg-background-tint-01 p-3">
                        <Text text03 secondaryBody>
                          OAuth passthrough is only available if you enable OIDC
