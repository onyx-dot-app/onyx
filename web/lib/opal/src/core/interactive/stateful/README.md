...
| `prominence`  | `"primary" \| "secondary" \| "tertiary"` | `"tertiary"` | Rest intensity — select-heavy only: `primary` rests on `background-neutral-01`, `secondary` on `background-tint-01`, `tertiary` transparent. Affects only the at-rest empty/filled backgrounds; other variants ignore it. |
