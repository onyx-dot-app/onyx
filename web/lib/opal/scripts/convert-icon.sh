...
# Convert an SVG file to a TypeScript React icon component.
# Icons are colour-overridable: stroke colours are stripped and replaced with currentColor.
#
# Usage (from the opal package root — web/lib/opal/):
#   ./scripts/convert-icon.sh src/icons/<filename.svg>

if [ -z "$1" ]; then
  echo "Usage: ./scripts/convert-icon.sh <filename.svg>" >&2
  exit 1
fi
...
# Run the conversion — strips stroke, stroke-opacity, width, and height attributes
