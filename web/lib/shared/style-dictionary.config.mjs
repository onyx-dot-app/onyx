...
StyleDictionary.registerFormat({
  ...
  format: ({ dictionary }) => {
    ...
    for (const t of dictionary.allTokens) {
      ...
      if (prop === "fontFamily") utilities[cls].fontFamily = firstFamily(val);
      else if (prop === "fontWeight") utilities[cls].fontWeight = String(val);
      else utilities[cls][prop] = `${toPx(val)}px`; // fontSize / lineHeight / letterSpacing
    }
    // Preserve a stable, RN-ish property order within each utility.
    const PROP_ORDER = [
      "fontFamily",
      "fontSize",
      "fontWeight",
    ...
    for (const cls of order) {
      ...
      for (const key of PROP_ORDER) {
        if (utilities[cls][key] !== undefined) decl[key] = utilities[cls][key];
      }
      ordered[cls] = decl;
    }
    return (
