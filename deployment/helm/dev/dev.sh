...
slugify() {
  # K8s name limit is 63 chars. Our longest rendered name is
  #   onyx-<slug>-celery-worker-scheduled-tasks-metrics
  # That's `onyx-` (5) + slug + `-celery-worker-scheduled-tasks-metrics` (38)
  # = 43 + slug. Cap slug at 19 to leave a 1-char safety margin under 63.
  # If the branch name is longer, the truncated slug is still recognizable
  # in `kubectl get ns` listings.
  local raw="$1"
  echo "${raw}" \
    | tr '[:upper:]' '[:lower:]' \
...
ensure_infra_release() {
  ...
  # Re-running helm upgrade on a healthy infra creates a server-side-apply
  # conflict with the redis-operator (which mutates the Redis CR's
  # resources.limits after install), leaving the release marked `failed`
  # while the cluster keeps running fine.
  #
  # We don't want to re-roll the infra on every per-worktree `dev.sh up`
  # anyway — it's the wrong surface for that. To force an infra upgrade
  # after chart edits, delete the release manually:
  #   helm -n onyx-infra uninstall onyx-infra
  if infra_ready; then
    log "infra is healthy; skipping helm upgrade of '${INFRA_RELEASE}'"
    return 0
  fi

  # OpenSearch password: use the hardcoded dev value from
  # values-dev-infra.yaml (it's local-only, lives in source). Both the
  # infra release and each per-worktree app release reference the same
  # string, so the per-worktree app's onyx-opensearch Secret matches the
  # actual deployed OpenSearch admin password.
  #
  # The --opensearch-password flag is still honored as an override for
  # devs who want a different value on first install.
  local pw_flag=()
...
write_worktree_values() {
  ...
  # Read the LIVE OpenSearch admin password from the infra release's
  # Secret. The per-worktree release creates its own onyx-opensearch
  # Secret; if its value diverges from what onyx-infra actually deployed,
  # the worktree's celery workers fail OpenSearch auth at startup. By
  # injecting the live value here, the worktree Secret always matches
  # the deployed OpenSearch regardless of how that password got there
  # (hardcoded in values-dev-infra.yaml, --opensearch-password flag, or
  # rotated by hand).
  local opensearch_password
  opensearch_password=$(kubectl -n "${INFRA_NS}" get secret onyx-opensearch \
    -o jsonpath='{.data.opensearch_admin_password}' 2>/dev/null \
...
cmd_up() {
  ...
  [[ "${slug}" =~ ^[a-z0-9-]+$ ]] && [[ "${#slug}" -le 19 ]] \
    || die "slug '${slug}' must match [a-z0-9-]+ and be ≤19 chars (k8s 63-char name limit)"
