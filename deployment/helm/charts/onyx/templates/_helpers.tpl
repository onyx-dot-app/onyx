...
{{/*
Env vars so Python HTTP clients (requests/httpx use certifi's bundle by
default) trust the system bundle regenerated by update-ca-certificates.
*/}}
...
{{/*
Items to prepend to an exec-form (JSON array) container command so that
update-ca-certificates runs before the real entrypoint. The original command
is passed through: sh assigns it to "$0" "$@" and exec's it.
Emits a single line ending with a comma, e.g.
