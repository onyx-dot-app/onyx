...
- Sessions: `POST /api/chat/create-chat-session {persona_id(=0 default), description, project_id}` → `{chat_session_id}`. Detail returns `messages[]` + `packets[][]` (replay) + `current_stream`. Resume in-flight: `GET /api/chat/chat-session/{id}/resume-stream?cursor=n`. Stop: `POST /api/chat/stop-chat-session/{id}`.
