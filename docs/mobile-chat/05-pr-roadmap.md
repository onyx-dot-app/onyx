...
| 2 | `feat(mobile): chat foundation — contracts + NDJSON parser + message tree + history` | ~550 | — | Pure contracts + parser + tree + history in `mobile/src/chat/`; unit-tested. **Mobile-local (not shared); web untouched.** |
...
## PR 2 — Mobile chat foundation: contracts + NDJSON parser + message tree  ✅ IMPLEMENTED (2026-06-29)
- **Goal:** Establish the pure, unit-tested core that PR 3's transport + UI build on. **Mobile-local — nothing extracted to `@onyx-ai/shared`; web is untouched** (see the decision override at the top of this doc).
