...
Skills delivery uses the shared `SandboxFilePusher` primitive documented in `../sandbox-file-push.md`. Skills builds bytes and calls; the primitive owns the wire protocol, NetworkPolicy, shared secret, fan-out, and atomic swap.

- **Admin mutations** (custom upload, bundle replace, grant change, `is_public` flip, builtin availability flip): the skills feature builds a per-user file dict and calls `pusher.push_to_users(tenant_id=..., mount_path="/workspace/managed/skills", user_files=...)`. A single-user grant change is a one-entry dict; an org-wide change (e.g. `is_public=True`) spans every affected user in the tenant.
- **Session start / wakeup**: the existing sandbox setup code calls `skills.push_to_pod(sandbox_id, user, db_session)`, which materializes the user's accessible skill set and pushes via `pusher.push_to_sandbox(...)`.
...
- Users without an active sandbox are silently skipped by the pusher.
...
- Cross-tenant isolation at push time is enforced by the pusher's label selector (`onyx.app/tenant-id`); the skills feature passes `tenant_id` explicitly per call.
...
- **Org-wide fan-out cost.** A public-skill mutation in a large tenant materializes per-user bundles for every user with an active sandbox. The pusher caps total bundle size at 100 MiB, but the skills-side cost of rebuilding the dict for hundreds of users is unmeasured. Acceptable for v1; revisit if it shows up in admin-mutation latency.
