...
1. `skills-requirements.md` — what V1 must do. Concept, bundle format, data model, visibility, sandbox-delivery model (S3-mounted bucket), API surface, non-requirements, open questions.
...
- `skills_plan.md` — long PRD / implementation spec; superseded by `skills-requirements.md` + `skills-api-plan.md`. Its sandbox-delivery design (per-session materialization + push pipeline) has been replaced by the S3-mounted-bucket model.
- `skills_plan.html` — HTML render of `skills_plan.md`; archived alongside it.
- `sandbox-file-sync.md` — push-pipeline / tarball / kubectl-exec delivery design; superseded by the S3-mount model in `skills-requirements.md` §5.
