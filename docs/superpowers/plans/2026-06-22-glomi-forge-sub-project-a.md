# glomi_forge 子项目 A 实现计划(Daytona + Pi 落地页端到端)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 搭建 `glomi_forge` 地基(领域模型 + Provider/Adapter 接口 + 编排状态机 + 1 个落地页模板),把「自然语言需求 → Daytona 沙箱 → Pi 构建 Next.js 落地页 → 预览 URL → 事件回流」端到端打通,与现有 opencode Craft 并行、feature flag 控制。

**Architecture:** Python 侧只做编排与中继。`ForgeOrchestrator` 驱动一个状态机:`SandboxProvider`(Daytona)起沙箱并暴露预览,`BuilderAdapter`(Pi)在沙箱内构建。沙箱内一个**我们自己的 launcher 脚本**把 Pi 的原始输出归一化成我们定义的 `ForgeEvent` JSONL 契约,Adapter 只依赖这个契约,不依赖 Pi 内部 schema。事件持久化进 `glomi_forge_event` 表(仿现有 `chat_run_event`:seq + JSONB),SSE 端点按 seq 轮询回放。
...
- **每个 LLM 调用**必须开 generation span 并用 `LLMFlow` 标签(本计划新增 `LLMFlow.FORGE_SPEC_GENERATION`)。
...
- **feature flag** `ENABLE_GLOMI_FORGE` 关闭时,新模块零副作用,现有 opencode Craft 行为不变。
...
backend/onyx/glomi_forge/
  ...
  schemas/
    __init__.py
    forge_spec.py                    # 新建:ForgeSpec / ForgeSpecInputs / ForgeSpecOutputs
    output_manifest.py               # 新建:OutputManifest / OutputFile / PreviewEntry
    forge_session.py                 # 新建:ForgeError + 领域视图模型
    events.py                        # 新建:ForgeEvent 契约(我们自己的事件)
  ...
  services/
    ...
    forge_spec_builder.py                  # 新建:ForgeSpecBuilder(NL→ForgeSpec)
    forge_orchestrator.py            # 新建:ForgeOrchestrator 状态机(核心)
  providers/
    __init__.py
  ...
  sandbox_image/
    ...
    run_forge.py                     # 新建:沙箱内 launcher(归一化 Pi → ForgeEvent JSONL)
    write_models_json.py             # 新建:从 env 写 ~/.pi/agent/models.json
backend/onyx/db/
  enums.py                           # 修改:加 GlomiForgeStatus / ForgeArtifactType 等
  models.py                          # 修改:加 GlomiForgeSession / GlomiForgeEvent 表
  glomi_forge.py                   # 新建:DB 操作
backend/onyx/error_handling/error_codes.py   # 修改:加 FORGE_* 错误码
backend/onyx/tracing/flows.py        # 修改:加 LLMFlow.FORGE_SPEC_GENERATION
backend/onyx/background/celery/tasks/glomi_forge/
  __init__.py                        # 新建
  tasks.py                           # 新建:run_forge_session_task
backend/onyx/server/features/glomi_forge/
  __init__.py
  api.py                             # 新建:路由(create/get/instruction/terminate/events SSE)
  sse.py                             # 新建:事件→SSE 帧
backend/onyx/main.py                 # 修改:include_router
backend/alembic/versions/<rev>_add_glomi_forge_tables.py   # 新建迁移
web/src/app/glomi-forge-dev/page.tsx       # 新建:最小内测触发页
web/src/hooks/useGlomiForgeSession.ts      # 新建:SWR hook
backend/tests/unit/glomi_forge/            # 新建:单元测试
backend/tests/external_dependency_unit/glomi_forge/   # 新建:PG/真实依赖测试
```
...
**Files:**
- Create: `backend/onyx/glomi_forge/__init__.py`(空)
- Create: `backend/onyx/glomi_forge/configs.py`
- Modify: `backend/pyproject.toml`(加 `daytona` 依赖)
- Create: `docs/superpowers/plans/A-spike-notes.md`

**Interfaces:**
- Produces: `ENABLE_GLOMI_FORGE: bool`、`DAYTONA_API_URL: str | None`、`DAYTONA_API_KEY: str | None`、`GLOMI_FORGE_DEFAULT_SNAPSHOT: str`、`LANDING_PAGE_TEMPLATE_ID: str = "landing_page"`,均从 `configs.py` 导出。

- [ ] **Step 1: 写 configs**

```python
# backend/onyx/glomi_forge/configs.py
...
```bash
git add backend/onyx/glomi_forge/ backend/pyproject.toml backend/uv.lock docs/superpowers/plans/A-spike-notes.md
git commit -m "feat(glomi_forge): module skeleton, configs, daytona dep, daytona spike notes"
```

---

## Task 2: 领域 Pydantic 模型 + ForgeEvent 契约

**Files:**
- Create: `backend/onyx/glomi_forge/schemas/__init__.py`, `forge_spec.py`, `output_manifest.py`, `forge_session.py`, `events.py`, `sandbox.py`, `builder.py`
- Test: `backend/tests/unit/glomi_forge/test_schemas.py`

**Interfaces:**
- Produces:
  - `ForgeSpec`(字段:`title:str`,`goal:str`,`target_audience:str|None`,`requirements:list[str]`,`constraints:list[str]`,`visual_style:list[str]`,`inputs:ForgeSpecInputs`,`outputs:ForgeSpecOutputs`,`acceptance_criteria:list[str]`)
  ...
  - `ForgeEvent`(Union,见下),每个成员有 `type:Literal[...]` + `at:str`
  - `ForgeError`(`code:str`,`message:str`,`retryable:bool`,`details:dict`,`occurred_at:str`)
...
```python
# backend/tests/unit/glomi_forge/test_schemas.py
from onyx.glomi_forge.schemas.forge_spec import ForgeSpec
from onyx.glomi_forge.schemas.events import (
...
Run: `source .venv/bin/activate && pytest backend/tests/unit/glomi_forge/test_schemas.py -v`
Expected: FAIL(`ModuleNotFoundError: onyx.glomi_forge.schemas.forge_spec`)

- [ ] **Step 3: 写 schemas**

```python
# backend/onyx/glomi_forge/schemas/forge_spec.py
...
```python
# backend/onyx/glomi_forge/schemas/output_manifest.py
from pydantic import BaseModel, Field
...
```python
# backend/onyx/glomi_forge/schemas/events.py
...
def parse_builder_event(raw: dict) -> ForgeEvent:
    return _ADAPTER.validate_python(raw)
```

```python
# backend/onyx/glomi_forge/schemas/forge_session.py
from pydantic import BaseModel, Field
...
```python
# backend/onyx/glomi_forge/schemas/sandbox.py
from pydantic import BaseModel, Field
...
```python
# backend/onyx/glomi_forge/schemas/builder.py
from pydantic import BaseModel, Field
...
Run: `pytest backend/tests/unit/glomi_forge/test_schemas.py -v`
Expected: PASS(3 passed)

- [ ] **Step 5: Commit**

```bash
git add backend/onyx/glomi_forge/schemas/ backend/tests/unit/glomi_forge/test_schemas.py
git commit -m "feat(glomi_forge): domain pydantic schemas + ForgeEvent contract"
```
...
- Create: `backend/alembic/versions/<rev>_add_glomi_forge_tables.py`
- Test: `backend/tests/external_dependency_unit/glomi_forge/test_models.py`

**Interfaces:**
- Produces: 枚举 `GlomiForgeStatus`、`ForgeArtifactType`;表 `GlomiForgeSession`(列见下)、`GlomiForgeEvent`(`id`,`session_id`,`seq`,`packet_json`,`created_at`)。

- [ ] **Step 1: 写失败测试**

```python
# backend/tests/external_dependency_unit/glomi_forge/test_models.py
from onyx.db.enums import GlomiForgeStatus, ForgeArtifactType
from onyx.db.models import GlomiForgeSession, GlomiForgeEvent
...
def test_tables_have_expected_columns():
    ...
    ev_cols = set(GlomiForgeEvent.__table__.columns.keys())
    assert {"id", "session_id", "seq", "packet_json"}.issubset(ev_cols)
```

- [ ] **Step 2: 跑测试确认失败**

Run: `pytest backend/tests/external_dependency_unit/glomi_forge/test_models.py -v`
Expected: FAIL(`ImportError: cannot import name 'GlomiForgeStatus'`)
...
class GlomiForgeSession(Base):
    """glomi_forge (Daytona+Pi) delivery session. Parallel to BuildSession
    (opencode Craft); gated by ENABLE_GLOMI_FORGE."""
    ...
    artifact_type: Mapped[ForgeArtifactType] = mapped_column(
        Enum(ForgeArtifactType, native_enum=False, name="buildartifacttype"),
        nullable=False,
    )
    ...
    status: Mapped[GlomiForgeStatus] = mapped_column(
        Enum(GlomiForgeStatus, native_enum=False, name="glomiforgestatus"),
        nullable=False,
...
class GlomiForgeEvent(Base):
    ...
    __table_args__ = (
        UniqueConstraint("session_id", "seq", name="uq_glomi_forge_event_session_seq"),
...
先 `cd backend && alembic revision -m "add glomi_forge tables"` 生成空文件,再手写内容(`down_revision` 用生成文件里已填好的值):
...
def upgrade() -> None:
    ...
    op.create_index("ix_glomi_forge_session_user", "glomi_forge_session", ["user_id"])
    op.create_table(
        "glomi_forge_event",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("session_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("glomi_forge_session.id", ondelete="CASCADE"), nullable=False),
        sa.Column("seq", sa.Integer(), nullable=False),
        sa.Column("packet_json", postgresql.JSONB(), nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.UniqueConstraint("session_id", "seq", name="uq_glomi_forge_event_session_seq"),
    )
    op.create_index("ix_glomi_forge_event_session_seq", "glomi_forge_event", ["session_id", "seq"])
...
pytest backend/tests/external_dependency_unit/glomi_forge/test_models.py -v
```
...
```bash
git add backend/onyx/db/enums.py backend/onyx/db/models.py backend/alembic/versions/ backend/tests/external_dependency_unit/glomi_forge/test_models.py
git commit -m "feat(glomi_forge): add GlomiForgeSession + GlomiForgeEvent tables, enums, migration"
```
...
**Files:**
- Create: `backend/onyx/db/glomi_forge.py`
- Test: `backend/tests/external_dependency_unit/glomi_forge/test_db.py`

**Interfaces:**
- Consumes: `GlomiForgeSession`,`GlomiForgeEvent`,`GlomiForgeStatus`,`ForgeArtifactType`,`ForgeSpec`,`ForgeError`,`OutputManifest`,`PreviewInfo`,`parse_builder_event`,`ForgeEvent`。
- Produces:
  - `create_glomi_forge_session(db, *, user_id, artifact_type, template_id, spec, title, parent_chat_session_id=None) -> GlomiForgeSession`
  - `get_glomi_forge_session(db, session_id) -> GlomiForgeSession | None`
  ...
  - `set_failed(db, session_id, error: ForgeError) -> None`
  - `append_build_event(db, session_id, event: ForgeEvent) -> int`(返回 seq)
  - `fetch_forge_events_after(db, session_id, after_seq: int) -> list[tuple[int, ForgeEvent]]`

- [ ] **Step 1: 写失败测试**(使用 `db_session` fixture,见 `backend/tests/external_dependency_unit/conftest.py`)

```python
# backend/tests/external_dependency_unit/glomi_forge/test_db.py
import uuid
from onyx.db.enums import ForgeArtifactType, GlomiForgeStatus
from onyx.glomi_forge.schemas.forge_spec import ForgeSpec
from onyx.glomi_forge.schemas.events import PreviewReady
from onyx.db.glomi_forge import (
    create_glomi_forge_session, get_glomi_forge_session, update_status,
    append_build_event, fetch_forge_events_after,
)


def test_create_and_status(db_session):
    s = create_glomi_forge_session(
        db_session, user_id=None, artifact_type=ForgeArtifactType.LANDING_PAGE,
        template_id="landing_page", spec=ForgeSpec(title="t", goal="g"), title="t",
    )
    update_status(db_session, s.id, GlomiForgeStatus.BUILDING)
    again = get_glomi_forge_session(db_session, s.id)
...
def test_events_seq_monotonic(db_session):
    ...
    rows = fetch_forge_events_after(db_session, s.id, after_seq=1)
    assert len(rows) == 1 and rows[0][1].port == 3001
```

- [ ] **Step 2: 跑测试确认失败**

Run: `pytest backend/tests/external_dependency_unit/glomi_forge/test_db.py -v`
Expected: FAIL(`ModuleNotFoundError: onyx.db.glomi_forge`)

- [ ] **Step 3: 实现 DB 操作**

```python
# backend/onyx/db/glomi_forge.py
"""DB operations for glomi_forge sessions and events.

All glomi_forge persistence goes through here (CLAUDE.md: db ops under onyx/db).
"""
import json
...
from onyx.glomi_forge.schemas.forge_spec import ForgeSpec
from onyx.glomi_forge.schemas.events import ForgeEvent, parse_builder_event
from onyx.glomi_forge.schemas.output_manifest import OutputManifest
from onyx.glomi_forge.schemas.sandbox import PreviewInfo
from onyx.db.enums import ForgeArtifactType, GlomiForgeStatus
from onyx.db.models import GlomiForgeEvent, GlomiForgeSession
...
def update_status(
    db_session: Session, session_id: UUID,
    status: GlomiForgeStatus, reason: str | None = None,
) -> None:
    ...
    if status == GlomiForgeStatus.COMPLETED:
...
def set_failed(db_session: Session, session_id: UUID, error: ForgeError) -> None:
    session = get_glomi_forge_session(db_session, session_id)
    if session:
...
def append_build_event(
    db_session: Session, session_id: UUID, event: ForgeEvent
) -> int:
    next_seq = (
        db_session.scalar(
...
```bash
git add backend/onyx/db/glomi_forge.py backend/tests/external_dependency_unit/glomi_forge/test_db.py
git commit -m "feat(glomi_forge): db operations for sessions and ordered events"
```
...
**Files:**
- Create: `backend/onyx/glomi_forge/interfaces/__init__.py`, `sandbox_provider.py`, `builder_adapter.py`
- Create: `backend/onyx/glomi_forge/testing/__init__.py`, `fakes.py`
- Test: `backend/tests/unit/glomi_forge/test_fakes.py`
...
- Produces:
  ...
  - `BuilderAdapter` Protocol:`start_build(input: StartBuildInput) -> StartBuildResult`、`subscribe(builder_session_id: str) -> Iterator[ForgeEvent]`、`stop(builder_session_id: str) -> None`
  - `FakeSandboxProvider`(可注入 preview),`FakeBuilderAdapter`(可注入 scripted events 列表)

- [ ] **Step 1: 写失败测试**

```python
# backend/tests/unit/glomi_forge/test_fakes.py
from onyx.glomi_forge.schemas.builder import StartBuildInput
from onyx.glomi_forge.schemas.events import BuilderFinished, PreviewReady
from onyx.glomi_forge.schemas.sandbox import CreateSandboxInput
from onyx.glomi_forge.testing.fakes import FakeBuilderAdapter, FakeSandboxProvider
...
Run: `pytest backend/tests/unit/glomi_forge/test_fakes.py -v`
Expected: FAIL(`ModuleNotFoundError: onyx.glomi_forge.testing.fakes`)

- [ ] **Step 3: 写 Protocol + Fakes**

```python
# backend/onyx/glomi_forge/interfaces/sandbox_provider.py
from typing import Protocol

from onyx.glomi_forge.schemas.sandbox import (
...
```python
# backend/onyx/glomi_forge/interfaces/builder_adapter.py
from collections.abc import Iterator
from typing import Protocol

from onyx.glomi_forge.schemas.builder import StartBuildInput, StartBuildResult
...
```python
# backend/onyx/glomi_forge/testing/fakes.py
...
class FakeBuilderAdapter:
    def __init__(self, scripted_events: list[ForgeEvent]):
...
```bash
git add backend/onyx/glomi_forge/interfaces/ backend/onyx/glomi_forge/testing/ backend/tests/unit/glomi_forge/test_fakes.py
git commit -m "feat(glomi_forge): SandboxProvider/BuilderAdapter protocols + fakes"
```
...
- Create: `backend/onyx/glomi_forge/services/__init__.py`, `template_service.py`
- Create: `backend/onyx/glomi_forge/templates/landing_page/AGENTS.md`, `SYSTEM.md`, `output_contract.md`
- Test: `backend/tests/unit/glomi_forge/test_template_service.py`

**Interfaces:**
- Produces:
  - `OnyxErrorCode.FORGE_PROVISION_FAILED = ("FORGE_PROVISION_FAILED", 502)`、`FORGE_BUILDER_FAILED = ("FORGE_BUILDER_FAILED", 502)`、`BUILD_SESSION_NOT_FOUND = ("BUILD_SESSION_NOT_FOUND", 404)`
  - `LLMFlow.FORGE_SPEC_GENERATION = "build_spec_generation"`
  - `TemplateDescriptor`(`template_id:str`,`artifact_type:ForgeArtifactType`,`snapshot:str`,`preview_port:int`,`agents_md:str`,`system_md:str`,`output_contract:str`)
  - `TemplateService.resolve(artifact_type: ForgeArtifactType) -> TemplateDescriptor`

- [ ] **Step 1: 写失败测试**

```python
# backend/tests/unit/glomi_forge/test_template_service.py
...
def test_resolve_landing_page():
    desc = TemplateService().resolve(ForgeArtifactType.LANDING_PAGE)
...
Run: `pytest backend/tests/unit/glomi_forge/test_template_service.py -v`
Expected: FAIL(`AttributeError: FORGE_PROVISION_FAILED` 或模块缺失)

- [ ] **Step 3a: 加错误码**(在 `error_codes.py` 合适分区追加)

```python
    # Build runtime (502 / 404)
    FORGE_PROVISION_FAILED = ("FORGE_PROVISION_FAILED", 502)
    FORGE_BUILDER_FAILED = ("FORGE_BUILDER_FAILED", 502)
...
```python
    FORGE_SPEC_GENERATION = "build_spec_generation"
```

- [ ] **Step 3c: 写模板资产**

`backend/onyx/glomi_forge/templates/landing_page/AGENTS.md`:
```markdown
...
`backend/onyx/glomi_forge/templates/landing_page/SYSTEM.md`:
```markdown
# System Notes
...
`backend/onyx/glomi_forge/templates/landing_page/output_contract.md`:
```markdown
# Output Contract
...
```python
# backend/onyx/glomi_forge/services/template_service.py
...
```bash
git add backend/onyx/error_handling/error_codes.py backend/onyx/tracing/flows.py backend/onyx/glomi_forge/services/ backend/onyx/glomi_forge/templates/ backend/tests/unit/glomi_forge/test_template_service.py
git commit -m "feat(glomi_forge): error codes, LLMFlow, TemplateService + landing_page assets"
```

---

## Task 7: ForgeOrchestrator 状态机(核心)

**Files:**
- Create: `backend/onyx/glomi_forge/services/forge_orchestrator.py`
- Test: `backend/tests/unit/glomi_forge/test_orchestrator.py`

**Interfaces:**
- Consumes: `SandboxProvider`,`BuilderAdapter`,`TemplateService`,Task 4 的所有 DB 函数,所有 schemas/events,`glomi_forge.configs`。
- Produces: `ForgeOrchestrator(db_session, provider, adapter, template_service)`,方法 `run(session_id: UUID) -> None`。`run` 内部:更新状态、起沙箱、写 context 文件、起 builder、消费 `subscribe()` 事件、`append_build_event` 持久化、遇 `PreviewReady` 调 `expose_preview` + `set_preview`、遇 `ArtifactReady` 读 manifest + `set_output`、遇 `BuilderFinished` 置 `completed`、遇 `BuilderFailed`/异常 `set_failed` + `delete_sandbox`。

- [ ] **Step 1: 写失败测试**

```python
# backend/tests/unit/glomi_forge/test_orchestrator.py
import uuid
import pytest
from onyx.db.enums import ForgeArtifactType, GlomiForgeStatus
from onyx.glomi_forge.schemas.forge_spec import ForgeSpec
...
from onyx.glomi_forge.services.template_service import TemplateService
from onyx.db.glomi_forge import (
    create_glomi_forge_session, get_glomi_forge_session, fetch_forge_events_after,
)
...
def test_happy_path_reaches_completed(db_session):
    ...
    ForgeOrchestrator(db_session, provider, adapter, TemplateService()).run(s.id)
    ...
    # events persisted (>=4)
    assert len(fetch_forge_events_after(db_session, s.id, after_seq=0)) >= 4
...
> 该测试用真实 PG(`db_session` fixture),因此放在 external_dependency。把文件移到 `backend/tests/external_dependency_unit/glomi_forge/test_orchestrator.py`,provider/adapter 仍是内存 fake。

- [ ] **Step 2: 跑测试确认失败**

Run: `pytest backend/tests/external_dependency_unit/glomi_forge/test_orchestrator.py -v`
Expected: FAIL(`ModuleNotFoundError: build_orchestrator`)

- [ ] **Step 3: 实现 orchestrator**

```python
# backend/onyx/glomi_forge/services/forge_orchestrator.py
"""Drives one glomi_forge session through its state machine.

Pure orchestration + relay: creates a sandbox via SandboxProvider, starts the
builder via BuilderAdapter, consumes the builder's ForgeEvent stream, persists
...
from onyx.glomi_forge.schemas.output_manifest import OutputManifest
from onyx.glomi_forge.schemas.sandbox import CreateSandboxInput, SandboxFile
from onyx.glomi_forge.services.template_service import TemplateService
from onyx.db.glomi_forge import (
    append_build_event, attach_builder, attach_sandbox, get_glomi_forge_session,
...
class ForgeOrchestrator:
    ...
    def run(self, session_id: UUID) -> None:
        session = get_glomi_forge_session(self.db, session_id)
        if session is None:
        ...
        try:
            ...
            update_status(self.db, session_id, GlomiForgeStatus.PROVISIONING)
            ...
            update_status(self.db, session_id, GlomiForgeStatus.BUILDING)
            ...
            for event in self.adapter.subscribe(start.builder_session_id):
                ...
                if isinstance(event, PreviewReady):
                    ...
                    update_status(self.db, session_id, GlomiForgeStatus.PREVIEW_READY)
                    ...
                    update_status(self.db, session_id, GlomiForgeStatus.COMPLETED)
                    return
    ...
    def _fail(self, session_id: UUID, message: str, sandbox_id: str | None) -> None:
        set_failed(
            self.db, session_id,
            ForgeError(code="FORGE_BUILDER_FAILED", message=message, occurred_at=_now()),
        )
        if sandbox_id:
            try:
    ...
    @staticmethod
    def _task_json(spec: ForgeSpec, template_id: str) -> dict:
        return {
            "template_id": template_id,
            "title": spec.title,
            "goal": spec.goal,
...
```bash
git add backend/onyx/glomi_forge/services/forge_orchestrator.py backend/tests/external_dependency_unit/glomi_forge/test_orchestrator.py
git commit -m "feat(glomi_forge): ForgeOrchestrator state machine (core, fake-tested)"
```

---

## Task 8: ForgeSpecBuilder(NL → ForgeSpec)

**Files:**
- Create: `backend/onyx/glomi_forge/services/forge_spec_builder.py`
- Test: `backend/tests/unit/glomi_forge/test_forge_spec_builder.py`(fake LLM)
- Test: `backend/tests/external_dependency_unit/glomi_forge/test_forge_spec_builder_llm.py`(真实 gpt-5-mini,gated)

**Interfaces:**
- Consumes: `ForgeSpec`,`LLMFlow.FORGE_SPEC_GENERATION`,`llm_generation_span`,`LLM` 接口(`invoke(..., structured_response_format=...)`)。
- Produces: `ForgeSpecBuilder(llm)`,方法 `build(nl_request: str, artifact_type: ForgeArtifactType) -> ForgeSpec`。

- [ ] **Step 1: 写失败测试(fake LLM)**

```python
# backend/tests/unit/glomi_forge/test_forge_spec_builder.py
...
def test_build_parses_structured_output():
    spec = ForgeSpecBuilder(_FakeLLM()).build("帮我做一个产品发布页", ForgeArtifactType.LANDING_PAGE)
...
Run: `pytest backend/tests/unit/glomi_forge/test_forge_spec_builder.py -v`
Expected: FAIL(模块缺失)

- [ ] **Step 3: 实现 ForgeSpecBuilder**

```python
# backend/onyx/glomi_forge/services/forge_spec_builder.py
"""Turns a natural-language delivery request into a structured ForgeSpec."""
from onyx.glomi_forge.schemas.forge_spec import ForgeSpec
from onyx.db.enums import ForgeArtifactType
...
class ForgeSpecBuilder:
    ...
    def build(self, nl_request: str, artifact_type: ForgeArtifactType) -> ForgeSpec:
        messages = [
            {"role": "system", "content": _SYSTEM},
            {"role": "user", "content": f"交付物类型:{artifact_type.value}\n需求:{nl_request}"},
        ]
        with llm_generation_span(
            llm=self.llm, flow=LLMFlow.FORGE_SPEC_GENERATION, input_messages=messages,
        ) as span:
...
```python
# backend/tests/external_dependency_unit/glomi_forge/test_forge_spec_builder_llm.py
import os
import pytest
from onyx.db.enums import ForgeArtifactType
from onyx.glomi_forge.services.forge_spec_builder import ForgeSpecBuilder
...
def test_real_llm_produces_valid_spec():
    ...
    spec = ForgeSpecBuilder(get_default_llm()).build(
        "帮我做一个面向年轻人的国潮咖啡品牌中文落地页", ForgeArtifactType.LANDING_PAGE
    )
    assert spec.title and spec.goal
```

Run: `python -m dotenv -f .vscode/.env run -- pytest backend/tests/external_dependency_unit/glomi_forge/test_forge_spec_builder_llm.py -v`
Expected: PASS(或无 key 时 skip)

- [ ] **Step 6: Commit**

```bash
git add backend/onyx/glomi_forge/services/forge_spec_builder.py backend/tests/unit/glomi_forge/test_forge_spec_builder.py backend/tests/external_dependency_unit/glomi_forge/test_forge_spec_builder_llm.py
git commit -m "feat(glomi_forge): ForgeSpecBuilder (NL->ForgeSpec) with FORGE_SPEC_GENERATION flow"
```

---

## Task 9: 沙箱镜像 + 沙箱内 launcher(归一化 Pi → ForgeEvent JSONL)

**Files:**
- Create: `backend/onyx/glomi_forge/sandbox_image/Dockerfile`
- Create: `backend/onyx/glomi_forge/sandbox_image/run_forge.py`
- Create: `backend/onyx/glomi_forge/sandbox_image/write_models_json.py`
- Test: `backend/tests/unit/glomi_forge/test_run_build_normalization.py`

**Interfaces:**
- Produces:
  - 一个 `glomi-landing-page` OCI 镜像(Node + bun + Next.js 模板 + Pi + python3),含 `/opt/glomi/run_forge.py`、`/opt/glomi/write_models_json.py`、预脚手架 `/workspace/src`。
  - `run_forge.py` 契约:读 `/workspace/input/task.json`,写 `~/.pi/agent/models.json`(从 env `GLOMI_LLM_BASE_URL/GLOMI_LLM_API_KEY/GLOMI_LLM_MODEL`),detached 跑 Pi,把 Pi 事件归一化成 ForgeEvent,**逐行 append 到 `/workspace/logs/events.jsonl`**;成功时起 `bun run dev`(port 3000)并写 `preview_ready` + `/workspace/out/output_manifest.json` + `artifact_ready` + `builder_finished`;失败写 `builder_failed`。
  - 可单元测试的纯函数 `normalize_pi_event(raw: dict) -> dict | None`(放 run_forge.py 内,import 可测)。

- [ ] **Step 1: 写失败测试(只测归一化纯函数)**

```python
# backend/tests/unit/glomi_forge/test_run_build_normalization.py
...
Run: `pytest backend/tests/unit/glomi_forge/test_run_build_normalization.py -v`
Expected: FAIL(文件不存在)

- [ ] **Step 3a: 写 launcher**

```python
# backend/onyx/glomi_forge/sandbox_image/run_forge.py
"""In-sandbox build launcher. Runs Pi, normalizes its raw JSON events into our
ForgeEvent JSONL contract, appended line-by-line to /workspace/logs/events.jsonl.
...
def normalize_pi_event(raw: dict) -> dict | None:
    """Map a raw Pi RPC/json event to our ForgeEvent dict, or None to drop."""
    t = raw.get("type")
    if t == "agent_start":
...
```python
# backend/onyx/glomi_forge/sandbox_image/write_models_json.py
...
```dockerfile
# backend/onyx/glomi_forge/sandbox_image/Dockerfile
...
# launcher scripts
COPY run_forge.py write_models_json.py /opt/glomi/
...
```bash
git add backend/onyx/glomi_forge/sandbox_image/ backend/tests/unit/glomi_forge/test_run_build_normalization.py
git commit -m "feat(glomi_forge): sandbox image + in-sandbox Pi launcher (normalized event contract)"
```
...
**Files:**
- Create: `backend/onyx/glomi_forge/providers/__init__.py`, `providers/sandbox/__init__.py`, `providers/sandbox/daytona_provider.py`
- Test: `backend/tests/unit/glomi_forge/test_daytona_provider.py`
...
```python
# backend/tests/unit/glomi_forge/test_daytona_provider.py
from types import SimpleNamespace
...
Run: `pytest backend/tests/unit/glomi_forge/test_daytona_provider.py -v`
Expected: FAIL(模块缺失)

- [ ] **Step 3: 实现 provider**

```python
# backend/onyx/glomi_forge/providers/sandbox/daytona_provider.py
...
from onyx.glomi_forge.configs import DAYTONA_API_KEY, DAYTONA_API_URL
from onyx.glomi_forge.schemas.sandbox import (
    CommandResult, CreateSandboxInput, CreateSandboxResult, PreviewInfo, SandboxFile,
)
from onyx.utils.logger import setup_logger
...
```bash
git add backend/onyx/glomi_forge/providers/ backend/tests/unit/glomi_forge/test_daytona_provider.py
git commit -m "feat(glomi_forge): DaytonaSandboxProvider (SDK-backed, mock-tested)"
```
...
**Files:**
- Create: `backend/onyx/glomi_forge/providers/builder/__init__.py`, `providers/builder/pi_builder_adapter.py`
- Test: `backend/tests/unit/glomi_forge/test_pi_adapter.py`
...
- Produces: `PiBuilderAdapter(provider, poll_interval=1.0, max_polls=...)`。`start_build`:`run_command` 后台跑 `python3 /opt/glomi/run_forge.py`(`nohup ... &`)。`subscribe`:循环 `read_file('/workspace/logs/events.jsonl')`,按已读行数增量 yield 新 `ForgeEvent`,遇 `builder_finished`/`builder_failed` 停。`stop`:`run_command` 杀进程。

- [ ] **Step 1: 写失败测试(假 provider,read_file 分两次返回递增内容)**

```python
# backend/tests/unit/glomi_forge/test_pi_adapter.py
from onyx.glomi_forge.schemas.builder import StartBuildInput
...
def test_subscribe_yields_until_finished():
    ...
    assert any("run_forge.py" in c for c in provider.commands)
...
Run: `pytest backend/tests/unit/glomi_forge/test_pi_adapter.py -v`
Expected: FAIL(模块缺失)

- [ ] **Step 3: 实现 adapter**

```python
# backend/onyx/glomi_forge/providers/builder/pi_builder_adapter.py
"""BuilderAdapter that drives the in-sandbox Pi launcher and tails its
normalized ForgeEvent JSONL. Depends only on our event contract + SandboxProvider."""
import time
from collections.abc import Iterator
...
class PiBuilderAdapter:
    ...
    def start_build(self, input: StartBuildInput) -> StartBuildResult:
        ...
        self.provider.run_command(
            input.sandbox_id,
            "mkdir -p /workspace/logs && nohup python3 /opt/glomi/run_forge.py "
...
```bash
git add backend/onyx/glomi_forge/providers/builder/ backend/tests/unit/glomi_forge/test_pi_adapter.py
git commit -m "feat(glomi_forge): PiBuilderAdapter (launcher + event tail)"
```
...
**Files:**
- Create: `backend/onyx/background/celery/tasks/glomi_forge/__init__.py`, `tasks.py`
- Modify: Celery 任务注册处(参考研究:任务模块需被 celery app 导入)
- Modify: `backend/onyx/configs/constants.py`(若 `OnyxCeleryTask` 枚举集中在此,加一个常量名)
- Test: `backend/tests/external_dependency_unit/glomi_forge/test_celery_task.py`

**Interfaces:**
- Consumes: `ForgeOrchestrator`,`DaytonaSandboxProvider`,`PiBuilderAdapter`,`TemplateService`,`get_session_with_current_tenant`。
- Produces: `@shared_task` `run_forge_session_task(self, *, session_id: str, tenant_id: str) -> None`;一个 helper `enqueue_forge_session(session_id: UUID, tenant_id: str) -> None`(用 `send_task(..., expires=...)`)。
...
```python
# backend/tests/external_dependency_unit/glomi_forge/test_celery_task.py
from onyx.db.enums import ForgeArtifactType, GlomiForgeStatus
from onyx.glomi_forge.schemas.forge_spec import ForgeSpec
from onyx.glomi_forge.schemas.events import BuilderStarted, BuilderFinished
from onyx.glomi_forge.testing.fakes import FakeBuilderAdapter, FakeSandboxProvider
from onyx.db.glomi_forge import create_glomi_forge_session, get_glomi_forge_session
...
def test_run_drives_orchestrator(db_session):
    ...
    assert get_glomi_forge_session(db_session, s.id).status == GlomiForgeStatus.COMPLETED
```

- [ ] **Step 2: 跑测试确认失败**

Run: `pytest backend/tests/external_dependency_unit/glomi_forge/test_celery_task.py -v`
Expected: FAIL(模块缺失)

- [ ] **Step 3: 实现任务**

```python
# backend/onyx/background/celery/tasks/glomi_forge/tasks.py
"""Celery task that drives a glomi_forge session end-to-end."""
from uuid import UUID
...
def _run(
    ...
    ForgeOrchestrator(db_session, provider, adapter, TemplateService()).run(session_id)


@shared_task(
    name="glomi_forge.run_session",
...
```python
# backend/onyx/background/celery/tasks/glomi_forge/__init__.py
"""glomi_forge celery tasks package. Imported by the celery app so the task
registers (mirror how server/features/build/sandbox/tasks is wired)."""
```

注册:在 celery app 的任务模块清单里加入 `onyx.background.celery.tasks.glomi_forge.tasks`(参照 `cleanup_idle_sandboxes_task` 现有注册方式)。enqueue helper(供 API 调用):
...
```bash
git add backend/onyx/background/celery/tasks/glomi_forge/ backend/tests/external_dependency_unit/glomi_forge/test_celery_task.py
git commit -m "feat(glomi_forge): celery task driving orchestrator + enqueue helper"
```
...
**Files:**
- Create: `backend/onyx/server/features/glomi_forge/__init__.py`, `api.py`, `sse.py`
- Modify: `backend/onyx/main.py`(include_router)
- Test: `backend/tests/external_dependency_unit/glomi_forge/test_api.py`(TestClient)

**Interfaces:**
- Consumes: `ForgeSpecBuilder`,`get_default_llm`,`create_glomi_forge_session`,`get_glomi_forge_session`,`fetch_forge_events_after`,`enqueue_forge_session`,`OnyxError`,`ENABLE_GLOMI_FORGE`,`require_permission`/`current_user`(沿用 build 现有 auth 依赖)。
- Produces:
  - `POST /api/glomi-forge/sessions` body `{request: str, artifact_type: str}` → 建 spec + session + enqueue,返回 `{session_id, status}`
  - `GET /api/glomi-forge/sessions/{id}` → 会话视图
  - `POST /api/glomi-forge/sessions/{id}/terminate`
  - `POST /api/glomi-forge/sessions/{id}/instruction` body `{content: str}`(A 期:记录 + 重新 enqueue,占位实现)
  - `GET /api/glomi-forge/sessions/{id}/events` → SSE,按 seq 回放 + keepalive

- [ ] **Step 1: 写失败测试**

```python
# backend/tests/external_dependency_unit/glomi_forge/test_api.py
import os, pytest
from fastapi.testclient import TestClient

pytestmark = pytest.mark.skipif(
    os.environ.get("ENABLE_GLOMI_FORGE", "").lower() != "true",
    reason="glomi_forge disabled",
)

def test_create_session_returns_id(monkeypatch):
    # stub ForgeSpecBuilder + enqueue so no real LLM/celery is needed
    from onyx.server.features.glomi_forge import api as mod
    from onyx.glomi_forge.schemas.forge_spec import ForgeSpec
    monkeypatch.setattr(mod, "_build_spec", lambda req, at: ForgeSpec(title="t", goal="g"))
    monkeypatch.setattr(mod, "enqueue_forge_session", lambda sid, tid: None)

    from onyx.main import app  # app factory
    client = TestClient(app)
    resp = client.post("/api/glomi-forge/sessions",
...
Run: `ENABLE_GLOMI_FORGE=true pytest backend/tests/external_dependency_unit/glomi_forge/test_api.py -v`
Expected: FAIL(模块缺失)

- [ ] **Step 3a: 写 SSE 编码**

```python
# backend/onyx/server/features/glomi_forge/sse.py
import json
from onyx.glomi_forge.schemas.events import ForgeEvent
...
```python
# backend/onyx/server/features/glomi_forge/api.py
import time
...
from onyx.glomi_forge.services.forge_spec_builder import ForgeSpecBuilder
from onyx.db.glomi_forge import (
    create_glomi_forge_session, fetch_forge_events_after, get_glomi_forge_session,
    update_status,
)
...
from onyx.llm.factory import get_default_llm
from onyx.server.features.glomi_forge.sse import SSE_KEEPALIVE, event_to_sse
...
def create_session(
    ...
    _guard()
    artifact_type = ForgeArtifactType(body.artifact_type)
    ...
    def gen() -> Generator[str, None, None]:
        ...
        while idle < 600:  # ~10 min safety bound
            ...
            with get_session_with_current_tenant() as db:
                rows = fetch_forge_events_after(db, session_id, after)
                session = get_glomi_forge_session(db, session_id)
            if rows:
...
```python
from onyx.server.features.glomi_forge.api import router as glomi_forge_router
include_router_with_global_prefix_prepended(application, glomi_forge_router)
```
...
```bash
git add backend/onyx/server/features/glomi_forge/ backend/onyx/main.py backend/tests/external_dependency_unit/glomi_forge/test_api.py
git commit -m "feat(glomi_forge): API routes (create/get/terminate/instruction/events SSE) + register"
```
...
**Files:**
- Create: `web/src/hooks/useGlomiForgeSession.ts`
- Create: `web/src/app/glomi-forge-dev/page.tsx`
- Test: 手动验证(前端 E2E 留给后续;A 期最小化)

**Interfaces:**
- Consumes: 后端 `POST /api/glomi-forge/sessions`、`GET /api/glomi-forge/sessions/{id}`。
- Produces: 一个内测页:输入中文需求 → 触发 → 轮询会话 → preview_url 出来后 iframe 展示。

- [ ] **Step 1: 写 SWR hook**

```typescript
// web/src/hooks/useGlomiForgeSession.ts
import useSWR from "swr";

interface GlomiForgeSessionView {
  session_id: string;
  status: string;
...
```tsx
// web/src/app/glomi-forge-dev/page.tsx
"use client";
...
export default function GlomiForgeDevPage() {
  ...
  const { session } = useGlomiForgeSession(sessionId);

  async function start() {
    const res = await fetch("/api/glomi-forge/sessions", {
      method: "POST",
...
```bash
git add web/src/hooks/useGlomiForgeSession.ts web/src/app/glomi-forge-dev/page.tsx
git commit -m "feat(glomi_forge): minimal dev trigger page + session polling hook"
```
...
**Files:**
- Create: `backend/tests/external_dependency_unit/glomi_forge/test_e2e_landing_page.py`

**Interfaces:**
- Consumes: 真实 `DaytonaSandboxProvider` + `PiBuilderAdapter` + `ForgeOrchestrator`,真实 `glomi-landing-page` snapshot(需先按 Task 9 构建并推到 Daytona)。

- [ ] **Step 1: 写 gated e2e**

```python
# backend/tests/external_dependency_unit/glomi_forge/test_e2e_landing_page.py
import os
import pytest
import requests
...
def test_landing_page_end_to_end(db_session):
    ...
    provider = DaytonaSandboxProvider()
    ForgeOrchestrator(db_session, provider, PiBuilderAdapter(provider), TemplateService()).run(s.id)

    session = get_glomi_forge_session(db_session, s.id)
    assert session.status == GlomiForgeStatus.COMPLETED
...
Run: `python -m dotenv -f .vscode/.env run -- pytest backend/tests/external_dependency_unit/glomi_forge/test_e2e_landing_page.py -v`
Expected: PASS(无 Daytona env 时 skip)

- [ ] **Step 3: Commit**

```bash
git add backend/tests/external_dependency_unit/glomi_forge/test_e2e_landing_page.py
git commit -m "test(glomi_forge): gated end-to-end landing page on real Daytona + Pi"
```
...
- Modify: `docs/GlomiAI.md`(B 路线表里标注 glomi_forge A 节点状态)
- Modify: `docs/superpowers/specs/2026-06-22-glomi-forge-roadmap.md`(勾选 A 实现完成)
...
uv run ty check backend/onyx/glomi_forge/
uv run pre-commit run --all-files
pytest backend/tests/unit/glomi_forge/ -v
```
...
```bash
git add summary.md docs/GlomiAI.md docs/superpowers/specs/2026-06-22-glomi-forge-roadmap.md
git commit -m "docs(glomi_forge): record sub-project A completion + learnings"
```
...
- 领域模型(ForgeSpec/OutputManifest/ForgeEvent/ForgeError/sandbox/builder)→ Task 2 ✅
- 新表 + 枚举 + 迁移(GlomiForgeSession/GlomiForgeEvent,避开 BuildSessionStatus 命名)→ Task 3 ✅
- DB 操作只放 onyx/db ✅(Task 4);事件表仿 chat_run_event ✅
- SandboxProvider/BuilderAdapter Protocol + fakes → Task 5 ✅
- 错误码 + LLMFlow.FORGE_SPEC_GENERATION + TemplateService + 落地页模板资产 → Task 6 ✅
- ForgeOrchestrator 状态机(核心,fake 测试,失败清理)→ Task 7 ✅
- ForgeSpecBuilder + 真实 LLM gated 测试(gpt-5-mini)→ Task 8 ✅
- 沙箱镜像 + launcher(Pi→ForgeEvent 归一化,我们自有契约)+ models.json(OpenAI-compatible→Pi openai)→ Task 9 ✅
