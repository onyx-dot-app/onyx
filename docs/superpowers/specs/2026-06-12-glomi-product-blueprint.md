...
| **E2** | 国产模型层 | 🟢 验证期 | DeepSeek/Qwen/智谱/Kimi/豆包 接入 + 默认优选;靠 XML tool-call 兼容无原生函数调用的模型 |
...
- **内容**：E1 i18n + E2 国产模型层 + E3 对话调优 + E4 深度研究中文化。
...
| 决策 | 现状 | 倾向/建议 |
|---|---|---|
| **模型策略** | 待定（你暂选 Other 未补充） | 验证期建议**纯国产 API**（最省、无 GPU 运维）;规模化/合规再议自部署开源模型 |
...
- **下一步建议**：沿验证路径推进 **E2 国产模型层** 出 spec（这是让产品"能用国产模型聊起来"的关键一步,验证期刚需）。
