...
- 搜什么来源。
- 何时 open_url。
- 如何处理 Lite / Medium / Deep 搜索强度。
- 如何把证据交给回答或深度研究流程。

因此本设计不是单纯“seed 一个 Tavily key”，而是把 Onyx 的 `web_search` 工具接入 Glomi 自己的 Search Gateway，并让 Agent 在工具调用中选择 `lite`、`medium` 或 `deep` 搜索模式。
...
6. `web_search` 工具支持 Agent 在同一次工具调用中选择 `mode=lite|medium|deep`。
...
11. 当 `open_url` 对某个搜索结果抓取失败，而最近一次 `web_search` 已经提供该 URL 的 snippet/raw-content 摘要时，`open_url` 可返回明确标注的 snippet fallback evidence，避免模型在页面被 403/反爬/超时拦截时完全失去证据。
...
- 不做关键词、正则、规则表来判断 Lite / Medium / Deep / Research。
...
medium      Agent 调 web_search(mode="medium")
...
`medium` 用于普通对话里的一次性研究型检索：

- 适合研究某个框架、项目、产品、公司、工具或中等复杂主题。
- 比 `lite` 多做少量 query fan-out，覆盖官方文档、GitHub / 发布记录、对比和限制等角度。
- Gateway 可以使用 Tavily advanced search 和有限 raw-content snippet fallback。
- 不进入多轮研究循环，也不自动生成长报告。
...
同时，`llm_loop` 已经会把最近 web_search 的 URL -> snippet 映射传给 `OpenURLTool`。如果 indexed retrieval、crawler 和 link-based lookup 都拿不到正文，`OpenURLTool` 会把匹配 URL 的 search snippet 转成一个可引用结果，并在内容中明确标注这是 recent `web_search` snippet fallback，不是完整页面正文。真实抓取内容仍然优先于 snippet fallback。
...
- 是否调用 `web_search`。
- 调用时使用 `mode=lite`、`mode=medium` 还是 `mode=deep`。
- 搜索后是否 `open_url`。
...
9. Gateway 错误状态映射清楚。
10. `WebSearchTool` 接受 `mode=lite|medium|deep`，并在缺省时使用 override default。
11. Deep Research / research agent 路径默认 `deep`。

Prompt tests：

1. 普通 `WEB_SEARCH_GUIDANCE` 包含 `mode=lite|medium|deep` 指导。
...
3. 普通 chat 的 Agent 能在工具调用中选择 `mode=lite`、`mode=medium` 或 `mode=deep`。
