...
> **E3/E4 联合决策**：超级对话调优和深度研究中文化不应拆成两套搜索逻辑。两者共享一层“中文 Agent 搜索与研究能力”：判断何时搜索、如何拆问题、生成什么 query、搜什么来源、串行还是并行、如何评估证据、如何把证据打包给 LLM。普通对话走轻量模式，深度研究走重型模式；这层能力未来也会成为 E13 的搜索型子能力。
