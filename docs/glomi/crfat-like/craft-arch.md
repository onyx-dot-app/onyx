...
# 《[GlomiAI](https://www.genspark.ai/api/files/s/QDdMm4Pf) × [Daytona](https://www.daytona.io/docs/architecture/) × [Pi](https://pi.dev/) 自研 Craft 架构草案 v0.1》

## 0. 文档定位

本文定义 GlomiAI 自研“Craft-like runtime”的第一版总体架构：  
...
对于交付物场景，系统自动创建 BuildSession，选择模板，启动 builder，在对话中同步进度，并在画布中展示预览与最终成品，而不是把用户跳转到一个独立的 Craft 产品页。[Source](https://www.genspark.ai/api/files/s/QDdMm4Pf)
...
用户体验上只有一个 GlomiAI。  
系统内部必须保留独立的 BuildSession、Sandbox、Workspace、Preview、Review、Publish 机制。
...
Daytona 官方架构将平台分为 interface plane、control plane、compute plane；其中 control plane 负责 sandbox 生命周期与资源调度，compute plane 负责 runner、sandbox daemon、snapshot、volume 等基础能力。这非常适合映射到 GlomiAI 的自研 Craft runtime 底座。[Source](https://www.daytona.io/docs/architecture/)

### 3.4 Pi 做 builder harness，不做总控

Pi 官方定位是 minimal agent harness，擅长 extensions、skills、prompt templates、SDK/RPC 等 agent 内核能力；同时它明确说明自己**没有 built-in sandbox**，真正隔离需要来自容器、VM、micro-VM 或远程 sandbox。因此 Pi 最适合放进 Daytona sandbox 中运行，担任单个 BuildSession 内的 builder agent。[Source](https://pi.dev/) [Source](https://pi.dev/docs/latest/security)
...
└──────────────────────────────────────────────┘
                     ...
                     ▼
┌──────────────────────────────────────────────┐
│         Build Orchestrator (Craft Core)      │
│ BuildSession / template / lifecycle / logs   │
└──────────────────────────────────────────────┘
           │                          │
           │                          │
           ▼                          ▼
...
用户提出需求
  ...
  -> 主控生成 BuildSpec
  -> 主控整理 Evidence Pack
  -> Build Orchestrator 创建 BuildSession
...
## 6.2 主控与 BuildSession 的关系

主控永远是用户可见主体。  
BuildSession 是后台任务实例。用户看到的是：
...
| GlomiAI BuildSession | Daytona Sandbox 状态 |
|---|---|
...
这些可以把 Pi 的执行过程稳定映射回 GlomiAI 的 timeline 和 BuildSession 状态。
...
Reviewer 的输出进入 `/review` 并回填到 BuildSession。
...
interface BuilderAdapter {
  ...
  subscribe(sessionId: string, onEvent: (event: BuilderEvent) => void): Unsubscribe
...
- **GlomiAI**：统一入口与主控编排层
- **Daytona**：自研 Craft 的 runtime substrate
