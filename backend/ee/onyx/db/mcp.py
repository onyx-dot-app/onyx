...
def make_mcp_server_private(
    ...
    """Reconcile the access rows for a server: clear existing user/group
    grants, then add the requested ones. Called with empty lists when a
    server is made public. Caller commits."""
    ...
    if user_ids:
        for user_uuid in user_ids:
            db_session.add(
                MCPServer__User(mcp_server_id=server_id, user_id=user_uuid)
            )
        ...
        for group_id in group_ids:
            db_session.add(
                MCPServer__UserGroup(
                    mcp_server_id=server_id, user_group_id=group_id
                )
            )
