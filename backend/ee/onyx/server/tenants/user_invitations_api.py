...
def _assert_tenant_is_joinable(email: str, tenant_id: str) -> None:
    ...
    # Case-folded: the forbidden list is lower case, so GMAIL.com would pass it.
    domain = email.split("@")[-1].casefold()
    tenant: TenantByDomainResponse | None = None
...
def request_invite(
    ...
    # Plain def: the control-plane lookup below blocks, so FastAPI must run
    # this route in a threadpool rather than on the event loop.
    ...
    try:
        ...
        raise OnyxError(OnyxErrorCode.INTERNAL_ERROR, "Failed to request invitation")
