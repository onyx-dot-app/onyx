...
def get_project_permissions(
    ...
    add_prefix: When True, prefix group IDs with `source` (for indexing path).
               When False (default), leave unprefixed (for permission sync path).
    source: Must be the DocumentSource stamped on the emitted documents so the
            group ids agree with the group sync, which prefixes by connector source.
    """
