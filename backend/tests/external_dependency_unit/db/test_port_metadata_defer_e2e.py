...
def test_deferred_metadata_sync_no_stale_permission_leak(
    ...
    # mimic document_index_metadata_sync_task's deferral branch
...
def test_port_batch_retry_does_not_revert_a_newer_metadata_update(
    ...
    copied, AFTER a live metadata sync applied a newer access to FUTURE. The
    retry's full-chunk write carries the SAME doc_updated_at as the first copy
    (it re-reads the same PRESENT chunk). External versioning must reject it as a
    benign 409 so the newer metadata update is NOT reverted.

    Unlike the existing index-vs-index `newest_wins` test, this exercises a
    *partial* metadata update (no external version) followed by an
    *external-versioned* full re-index — the interleaving the reindex port can hit
    when a metadata sync lands between two retry attempts of the same batch.
    """
    _, doc_id, _present_client, future_client, _present_name, future_name = env
    future_index = _index(future_name)
    pinned_ts = datetime(2026, 6, 1, tzinfo=timezone.utc)  # stable external version

    # 1. Port copies the chunk into FUTURE with OLD access (doc_updated_at=pinned).
    future_client.bulk_index_documents(
    ...
    # 3. Port BATCH RETRY re-writes the SAME chunk (OLD access, SAME doc_updated_at).
    future_client.bulk_index_documents(
        documents=[_make_chunk(doc_id, 0, _ACCESS_OLD, last_updated=pinned_ts)],
        tenant_state=_TENANT_STATE,
        use_external_versioning=True,
