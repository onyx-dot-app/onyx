...
def test_put_rejects_lockdown_toggle_on_non_basic_auth(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The lockdown toggles only apply to password (BASIC) deployments. Setting
    one elsewhere would silently never enforce, so the write is refused."""
    from onyx.configs.constants import AuthType

    monkeypatch.setattr(security_api, "AUTH_TYPE", AuthType.CLOUD)
