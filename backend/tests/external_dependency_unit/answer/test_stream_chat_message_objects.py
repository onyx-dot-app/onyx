...
def test_stream_chat_message_objects_without_web_search(
    ...
    # Apply the mock for the duration of this test
    with patch("requests.post", side_effect=mock_post):
        # Also mock the EmbeddingModel's _make_model_server_request method directly
        # to ensure we catch all embedding calls
        with patch(
            ...
            test_user: User = create_test_user(
                db_session, email_prefix="test_web_search"
            )
