...
def test_mcp_server_health_check(reset: None) -> None:  # noqa: ARG001, W291
...
def test_mcp_server_auth_missing_token(reset: None) -> None:  # noqa: ARG001, W291
    """Test MCP server rejects requests without credentials."""
    response = requests.post(STREAMABLE_HTTP_URL)
    assert response.status_code == 401


def test_mcp_server_auth_invalid_token(reset: None) -> None:  # noqa: ARG001, W291
