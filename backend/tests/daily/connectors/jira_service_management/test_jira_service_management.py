...
def _test_jsm_connector_basic(
    ...
    jsm_docs = [d for d in docs if d.metadata.get("request_status")]
    assert len(jsm_docs) > 0, "Expected at least one doc with JSM request_status metadata"
    ...
    assert section.text == "This is a critical request for super-human answer quality in Onyx! We need magic!\n"
