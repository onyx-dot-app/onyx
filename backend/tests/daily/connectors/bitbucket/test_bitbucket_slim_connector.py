...
def bitbucket_connector_for_slim() -> BitbucketConnector:
    ...
    projects = os.environ.get("BITBUCKET_PROJECTS", "PEAL")
    prune_days = -1
    ...
    connector.load_credentials({"email": email, "api_token": token})
    return connector
...
def test_bitbucket_full_ids_subset_of_slim_ids(
    ...
    all_full_doc_ids: set[str] = set()
    for doc_batch in bitbucket_connector_for_slim.load_from_state():
