...
from onyx.file_processing.extract_file_text import DOCUMENT_FILE_EXTENSIONS
from onyx.file_processing.extract_file_text import get_file_ext
from onyx.file_processing.extract_file_text import IMAGE_FILE_EXTENSIONS
from onyx.file_processing.extract_file_text import PLAIN_TEXT_FILE_EXTENSIONS
...
def test_blob_s3_connector(
    ...
    for doc in all_docs:
        ...
        if file_extension in PLAIN_TEXT_FILE_EXTENSIONS:
            assert len(section.text) > 0
            continue

        if file_extension in DOCUMENT_FILE_EXTENSIONS:
            assert len(section.text) > 0
            continue
