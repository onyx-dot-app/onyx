...
class _StreamingToolUnsupportedLLM(_ConfigOnlyLLM):
    def __init__(self) -> None:
        super().__init__(
            LLMConfig(
...
def test_streaming_tool_error_reports_model_as_unsupported_without_retry() -> None:
    with patch.object(gateway_api, "llm_generation_span", return_value=nullcontext()):
        frames = list(
            _gateway_stream(
                _StreamingToolUnsupportedLLM(),
                tools=[{"type": "function"}],
            )
        )
    ...
    assert error == {
        "message": (
            "This model does not support streaming tool use, which is required "
            "by Craft. Please choose a different model."
        ),
