...
def _prepuller_pod(
    ref: str, image_id: str | None, node: str | None = "node-a"
) -> client.V1Pod:
    pod = SimpleNamespace(
        metadata=SimpleNamespace(name="prepuller-abc", labels={}),
        spec=SimpleNamespace(
            node_name=node,
            containers=[SimpleNamespace(name="prepuller", image=ref)],
        ),
...
def _sandbox_pod(
    sandbox_id: UUID | str, image_id: str | None, node: str | None = "node-a"
) -> client.V1Pod:
    pod = SimpleNamespace(
...
def _k8s(prepullers: list[client.V1Pod], sandboxes: list[client.V1Pod] | None = None):
    ...
    mgr._prepull_cache = None  # type: ignore[attr-defined]
    return mgr, core_api
...
def test_agreed_prepull_gives_a_digest_pinned_target() -> None:
    ...
    mgr, _ = _k8s(
        [
            _prepuller_pod(TAG_REF, f"docker.io/onyxdotapp/sandbox@{DIGEST_A}", node)
...
def test_disagreeing_nodes_yield_no_target() -> None:
    ...
    mgr, _ = _k8s(
        [
            _prepuller_pod(TAG_REF, DIGEST_A, "node-a"),
            _prepuller_pod(TAG_REF, DIGEST_B, "node-b"),
        ]
    )
...
def test_node_still_pulling_does_not_stall_the_others() -> None:
    """A node mid-pull leaves its own sandboxes alone, but must not hold up nodes
    that have the image — with restarts on independent timers, some node is
    usually pulling."""
    mgr, _ = _k8s(
        [
            _prepuller_pod(TAG_REF, DIGEST_A, "node-a"),
            _prepuller_pod(TAG_REF, None, "node-b"),
        ],
        [
            _sandbox_pod(SANDBOX_A, DIGEST_B, "node-a"),
            _sandbox_pod(SANDBOX_B, DIGEST_B, "node-b"),
        ],
    )

    state = mgr.get_image_state()

    assert state.target is not None and state.target.digest == DIGEST_A
    assert state.stale_sandbox_ids() == {SANDBOX_A}
...
def test_fleet_digests_come_from_one_list_call() -> None:
    ...
    digests = mgr.get_image_state().movable_digests
...
def test_fleet_digests_skip_pods_that_cannot_be_tied_to_a_sandbox() -> None:
    ...
    assert mgr.get_image_state().movable_digests == {SANDBOX_B: DIGEST_B}
...
def test_fleet_digests_omit_pods_that_have_not_reported() -> None:
    ...
    mgr, _ = _k8s([_prepuller_pod(TAG_REF, DIGEST_B)], [_sandbox_pod(SANDBOX_A, None)])
...
def test_docker_fleet_digests_use_the_list_shape() -> None:
    ...
    assert mgr.get_image_state().movable_digests == {SANDBOX_A: DIGEST_A}
...
def test_sandbox_on_a_node_without_a_prepuller_is_not_movable() -> None:
    """The rollout hole: a DaemonSet deletes a pod before creating its
    replacement, and a new node has none until the controller catches up. Vouching
    for that node on the strength of its neighbours would patch a sandbox onto an
    image its host lacks — which takes it down until the pull finishes, and the
    rebuild fallback cannot snapshot an unreachable pod.
    """
    mgr, _ = _k8s(
        [_prepuller_pod(TAG_REF, DIGEST_B, "node-a")],
        [
            _sandbox_pod(SANDBOX_A, DIGEST_A, "node-a"),
            _sandbox_pod(SANDBOX_B, DIGEST_A, "node-b"),
        ],
    )

    state = mgr.get_image_state()

    assert state.target is not None and state.target.digest == DIGEST_B
    assert state.stale_sandbox_ids() == {SANDBOX_A}


def test_a_sandbox_becomes_movable_once_its_node_reports() -> None:
    """Self-healing: nothing is recorded, so the next pass picks it up."""
    mgr, core_api = _k8s(
        [_prepuller_pod(TAG_REF, DIGEST_B, "node-a")],
        [_sandbox_pod(SANDBOX_B, DIGEST_A, "node-b")],
    )
    assert mgr.get_image_state().stale_sandbox_ids() == set()

    mgr._prepull_cache = None  # type: ignore[attr-defined]
    core_api.list_namespaced_pod.side_effect = lambda *_a, **kwargs: SimpleNamespace(
        items=(
            [
                _prepuller_pod(TAG_REF, DIGEST_B, "node-a"),
                _prepuller_pod(TAG_REF, DIGEST_B, "node-b"),
            ...
            else [_sandbox_pod(SANDBOX_B, DIGEST_A, "node-b")]
        )
    )

    assert mgr.get_image_state().stale_sandbox_ids() == {SANDBOX_B}
