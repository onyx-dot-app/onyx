...
def test_full_history_incognito_still_traces() -> None:
    """FULL_HISTORY records like an ordinary chat, tracing included."""
    logger = MagicMock()
    processor = BraintrustTracingProcessor(logger=logger)

    token = CURRENT_INCOGNITO_RECORD_MODE_CONTEXTVAR.set("full_history")
    try:
        processor.on_trace_start(_fake_trace("t3"))
    finally:
        CURRENT_INCOGNITO_RECORD_MODE_CONTEXTVAR.reset(token)
