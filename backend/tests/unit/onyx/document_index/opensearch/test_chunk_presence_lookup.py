...
`get_existing_chunk_ids` reads by _id through mget rather than searching, so a
chunk written but not yet refreshed still counts as present; these pin the
batching and the found/not-found split. `get_documents_missing_chunks` builds on
it to answer "is this document in the index at all", which is what the port-flow
swap gate samples before promoting a new index.
"""
...
def _make_index(found_chunk_ids: set[str]) -> tuple[OpenSearchDocumentIndex, MagicMock]:
    """An index whose client reports exactly `found_chunk_ids` as present."""
    ...
    client = MagicMock()
    client.get_existing_chunk_ids.side_effect = lambda chunk_ids, **_: {
...
def _make_client_with_mget(present_ids: set[str]) -> Any:
    """A real OpenSearchIndexClient with only its transport stubbed."""
...
def test_get_existing_chunk_ids_batches_large_inputs() -> None:
    ...
    assert client.get_existing_chunk_ids(chunk_ids, batch_size=500) == set(chunk_ids)
