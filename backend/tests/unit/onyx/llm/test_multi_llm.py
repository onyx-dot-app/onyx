...
def _bedrock_llm(model_name: str) -> LitellmLLM:
    return LitellmLLM(
        api_key=None,
        timeout=30,
...
@pytest.mark.parametrize("model_name", BEDROCK_OPENAI_GPT_MODELS)
def test_bedrock_openai_gpt_omits_temperature(
    model_name: str, is_reasoning: bool
) -> None:
    ...
    with (
        ...
        list(_bedrock_llm(model_name).stream([UserMessage(content="Hi")]))

        assert "temperature" not in mock_completion.call_args.kwargs


@pytest.mark.parametrize("model_name", BEDROCK_OPENAI_GPT_MODELS)
def test_bedrock_openai_gpt_sends_openai_style_reasoning(model_name: str) -> None:
    # LiteLLM's reasoning_effort becomes Anthropic thinking for these, which
    # Converse rejects; reasoning.effort is what they take.
    with (
        patch("litellm.completion") as mock_completion,
        patch("onyx.llm.multi_llm.model_is_reasoning_model", return_value=True),
    ):
        ...
        list(
            _bedrock_llm(model_name).stream(
                [UserMessage(content="Hi")], reasoning_effort=ReasoningEffort.HIGH
            )
        )
...
def test_bedrock_gpt_oss_keeps_temperature_and_reasoning_effort() -> None:
    with (
        patch("litellm.completion") as mock_completion,
        patch("onyx.llm.multi_llm.model_is_reasoning_model", return_value=True),
    ):
        ...
        list(
            _bedrock_llm("openai.gpt-oss-120b-1:0").stream(
                [UserMessage(content="Hi")], reasoning_effort=ReasoningEffort.HIGH
            )
        )
