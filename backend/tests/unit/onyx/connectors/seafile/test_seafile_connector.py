...
from onyx.connectors.capability_checks.recorder import _outcome_status
...
from onyx.connectors.seafile.connector import (
    SEAFILE_API_TOKEN_KEY,
    SEAFILE_SUPPORTED_EXTENSIONS,
    SeafileCheckpoint,
    SeafileConnector,
    _modified_time,
)
...
def _connector(max_file_size_bytes: int = 100) -> SeafileConnector:
    ...
    connector.set_raw_file_callback(lambda _content, _content_type: "staged-file")
    return connector
...
def test_binary_files_use_onyx_file_parsers() -> None:
    ...
    def fake_tabular_file_to_sections(
        *, file: Any, file_name: str, stage: Any, link: str
    ) -> list[TabularSection]:
        _ = file, stage
        parser_calls.append(file_name)
...
def test_rate_limit_is_reported_cleanly() -> None:
    ...
    with patch(
        ...
        with pytest.raises(UnexpectedValidationError, match="rate limit") as exc_info:
            _drain_checkpoint(connector)
    assert _outcome_status(exc_info.value) == CapabilityCheckStatus.INDETERMINATE
...
def test_server_error_is_reported_with_context() -> None:
    ...
    with patch(
        ...
        with pytest.raises(
            UnexpectedValidationError, match="get library metadata repo=repo-1"
        ) as exc_info:
            _drain_checkpoint(connector)
    assert _outcome_status(exc_info.value) == CapabilityCheckStatus.INDETERMINATE
...
def test_timeout_is_reported_cleanly() -> None:
    ...
    with patch(
        ...
        with pytest.raises(UnexpectedValidationError, match="timed out") as exc_info:
            _drain_checkpoint(connector)
    assert _outcome_status(exc_info.value) == CapabilityCheckStatus.INDETERMINATE
...
def test_malformed_json_is_reported_with_context() -> None:
    ...
    with patch(
        ...
        with pytest.raises(
            UnexpectedValidationError, match="malformed JSON"
        ) as exc_info:
            _drain_checkpoint(connector)
    assert _outcome_status(exc_info.value) == CapabilityCheckStatus.INDETERMINATE
