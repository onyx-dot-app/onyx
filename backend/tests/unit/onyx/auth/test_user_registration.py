...
class TestPasswordSignupLockdown:
    ...
    (``safe=True``). SAML/JWT provisioning uses the default ``safe=False`` and
    OAuth bypasses create(), so SSO users are still created through their
    provider.
    """
...
class TestPasswordLoginLockdown:
    ...
    with a distinct reason, single-tenant only (multi-tenant would resolve
    settings before the user's tenant is known, so the gate must not fire
    there). SSO never reaches authenticate()."""
