...
def test_lazy_import_settings_ignore_files(tmp_path: Path) -> None:
    """Test that LazyImportSettings.ignore_files properly excludes specified files."""
    # Create a temporary backend directory structure
    backend_dir = tmp_path / "backend"
    backend_dir.mkdir()

    # Create a file that would normally have violations
    violation_file = backend_dir / "violation_module.py"
    violation_file.write_text(
        """
import vertexai
...
"""
    ...
    # Create a file that should be ignored via settings
    ignored_file = backend_dir / "special_module.py"
...
"""
    ...
    another_ignored = backend_dir / "config" / "setup.py"
    another_ignored.parent.mkdir(parents=True)
    another_ignored.write_text(
        """
import vertexai
import nltk
"""
    ...
    with patch("scripts.check_lazy_imports.__file__", str(script_path)):
        # Test without ignore_files - should find violations in all files
        with pytest.raises(
            RuntimeError,
            match="Found eager imports of .+\\. You must import them only when needed",
        ):
        ...
        # Test with ignore_files - should only find violations in non-ignored files
        with pytest.raises(
            RuntimeError,
            match="Found eager imports of .+\\. You must import them only when needed",
        ):
        ...
        # Create a test where ALL violation files are ignored - should pass
        violation_file.unlink()  # Remove the non-ignored violation file

        # Now only ignored files have violations, so main should not raise
        main(
            {
                "vertexai": LazyImportSettings(
                    ignore_files={"special_module.py", "config/setup.py"}
...
def test_lazy_import_settings_ignore_files_pattern_matching(tmp_path: Path) -> None:
    """Test that ignore_files patterns match correctly against file paths."""
    # Create a temporary backend directory structure
    backend_dir = tmp_path / "backend"
    backend_dir.mkdir()

    # Create nested structure with violations
    nested_dir = backend_dir / "onyx" / "connectors"
    nested_dir.mkdir(parents=True)

    factory_file = nested_dir / "factory.py"
...
"""
    ...
    # Create another file in different location
    other_dir = backend_dir / "onyx" / "llm"
    other_dir.mkdir(parents=True)

    other_file = other_dir / "get_litellm.py"
...
"""
    ...
    with patch("scripts.check_lazy_imports.__file__", str(script_path)):
        # Test with specific file path in ignore - should ignore only that file
        with pytest.raises(
            RuntimeError,
            match="Found eager imports of .+\\. You must import them only when needed",
        ):
        ...
        # Test with both files ignored - should pass
        main(
            {
                "vertexai": LazyImportSettings(
                    ignore_files={
        ...
        # Test with partial path matching
        main(
            {
                "vertexai": LazyImportSettings(
                    ignore_files={
