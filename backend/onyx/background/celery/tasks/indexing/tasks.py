...
def connector_document_extraction_task(
    ...
    try:
        with get_session_with_current_tenant() as db_session:
            ...
            # }
            # batch_storage.store_extraction_state(state)
        ...
        while checkpoint.has_more:
            ...
            for document_batch, failure, next_checkpoint in connector_runner.run(
                ...
                batch_id = f"batch_{batch_num}_{int(time.time())}"

                # Store documents in storage
                batch_storage.store_batch(batch_id, doc_batch_cleaned)

                # Create processing task data
                is_final_batch = not checkpoint.has_more and not next_checkpoint
