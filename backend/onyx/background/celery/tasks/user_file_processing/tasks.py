...
def check_for_user_file_project_sync(self: Task, *, tenant_id: str) -> None:
    ...
    try:
        ...
        if queue_depth > USER_FILE_PROJECT_SYNC_MAX_QUEUE_DEPTH:
            task_logger.warning(
                f"check_for_user_file_project_sync - Queue depth {queue_depth} exceeds "
    ...
    task_logger.info(
        f"check_for_user_file_project_sync - Enqueued {enqueued} "
        f"skipped_guard={skipped_guard} tasks for tenant={tenant_id}"
    )
    return None
