"""Map stored provider settings to request-scoped SDK arguments.

Provider credentials never modify the process environment.
"""
...
# Both Bedrock variants accept explicit credentials or an API bearer token.
...
# Map custom_config keys to SDK arguments. Both the argument
...
def _map_vertex_config(custom_config: dict[str, str]) -> CustomConfigMapping:
    ...
    for key, value in custom_config.items():
        if key == VERTEX_AUTH_METHOD_KWARG:
            ...
            consumed.add(key)
            # Workload Identity uses ambient Google credentials.
...
def map_custom_config_to_model_kwargs(
    ...
    """Translate stored settings into explicit SDK arguments.

    Provider-level `api_key` / `api_base` win over the generic
    `<PROVIDER>_API_KEY` / `<PROVIDER>_API_BASE` config keys, mirroring
    the existing setting precedence. Provider-specific keys (e.g.
...
def get_unsupported_custom_config_keys(
    ...
    """Return custom configuration keys without a native provider mapping."""
    if not custom_config:
        return set()
