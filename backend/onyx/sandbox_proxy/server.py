...
def main() -> int:
    ...
    try:
        ...
        async def _async_main() -> None:
            ...
            if SANDBOX_PROXY_ALLOW_GLOBAL_CLIENTS:
                # `block_global` belongs to the `block` addon, so it can only
                # be set once DumpMaster has registered its addons. See the
                # config for why IPv6-only clusters need this.
