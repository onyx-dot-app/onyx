...
/ ``K8sInformerLookup`` (or their docker analogues) directly so the
proxy boots correctly under either ``SANDBOX_BACKEND`` value.

Imports of the K8s and docker concrete classes are deferred to the
dispatch call. This keeps each backend's heavy import (``kubernetes``
or ``docker``) out of the other backend's process import graph, which
matters at proxy boot where the wrong-backend SDK opening config files
on import would be wasted work at best and crash-on-missing-config at
worst.
"""
