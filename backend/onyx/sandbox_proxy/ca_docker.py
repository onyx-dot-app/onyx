...
A shared compose-named volume is the source of truth: the proxy mounts
it read-write so it can persist on cold start; every sandbox container
...
raises rather than silently regenerating (which would invalidate
trust stores already populated from the orphaned cert). Operator
recovery is: delete ``ca.crt``, restart proxy.
"""
...
class FileCAStore(CAStore):
    ...
    ``persist`` is idempotent under concurrent callers via ``O_EXCL`` on
    the cert (the rendezvous file). The key is written second; if a
    crash interrupts between the two, the next boot fails loud rather
    than regenerating and invalidating trust stores.
    """
    ...
    def load(self) -> tuple[bytes, bytes] | None:
        ...
        if cert_exists and not key_exists:
            # Half-written state from a crash mid-persist. Fail loud so
            # the operator can recover (delete ca.crt, restart) rather
            # than silently regenerating and orphaning the cert that
            # sandboxes may have already trusted.
            raise RuntimeError(
                f"proxy CA cert exists at {self._cert_path} but key is missing "
                f"at {self._key_path}; refusing to regenerate. "
                f"Recovery: delete {self._cert_path} and restart the proxy."
            )
        if not cert_exists and key_exists:
            # Symmetric anomaly: key without cert. Same fail-loud posture.
            raise RuntimeError(
                f"proxy CA key exists at {self._key_path} but cert is missing "
                f"at {self._cert_path}; refusing to regenerate. "
                f"Recovery: delete {self._key_path} and restart the proxy."
            )
    ...
    def persist(self, cert_pem: bytes, key_pem: bytes) -> None:
        ...
        # Cert first: it's the rendezvous file. Losing the O_EXCL race
        # here means another replica won; raise so CABootstrap re-load()s
        # the winner's CA.
        try:
            fd = os.open(
                self._cert_path,
        ...
        # Key second. If this fails the cert is orphaned -- next load()
        # raises the half-written-state error above.
        fd = os.open(
            self._key_path,
