...
class FreshdeskKnowledgeBaseConnector(LoadConnector, PollConnector, SlimConnector):
    ...
    def load_credentials(self, credentials: dict[str, str | int]) -> None:
        ...
        portal_url = credentials.get(
            "freshdesk_portal_url"
        )  # For constructing agent URLs
        portal_id = credentials.get(
            "freshdesk_portal_id"
        )  # For constructing agent URLs

        # Check credentials
        if not all(
            isinstance(cred, str) for cred in [domain, api_key] if cred is not None
        ):
            missing = [
                name
                for name, val in {
    ...
    def validate_connector_settings(self) -> None:
        ...
        if hasattr(self, "folder_ids"):
            if isinstance(self.folder_ids, list):
                ...
                parsed_ids = [
                    fid.strip() for fid in self.folder_ids.split(",") if fid.strip()
                ]
                folder_ids.extend(parsed_ids)

        # Also check connector_specific_config directly
        if (
            self.connector_specific_config
            and "freshdesk_folder_ids" in self.connector_specific_config
        ):
            ...
            if isinstance(folder_ids_value, list):
                ...
                parsed_ids = [
                    fid.strip() for fid in folder_ids_value.split(",") if fid.strip()
                ]
                folder_ids.extend(parsed_ids)
        ...
        if not folder_ids:
            ...
            if (
                hasattr(self, "connector_specific_config")
                and self.connector_specific_config
            ):
    ...
    def list_available_folders(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for category in categories:
                ...
                logger.info(
                    f"Found {len(folders)} folders in category '{category_name}'"
                )
    ...
    def _fetch_articles_from_folder(
        ...
        while True:
            ...
            if updated_since:
                ...
                if filtered_batch:
                    logger.info(
                        f"Fetched {len(filtered_batch)} articles updated since {updated_since.isoformat()} from folder {folder_id}, page {page}."
                    )
                    yield filtered_batch
            else:
                logger.info(
                    f"Fetched {len(article_batch)} articles from folder {folder_id}, page {page}."
                )
                yield article_batch
            ...
            for folder_id in folder_ids:
                ...
                for article_list_from_api in self._fetch_articles_from_folder(
                    ...
                    for article_data in article_list_from_api:
                        try:
                            doc = _create_doc_from_article(
                                article_data, self.domain, portal_url, portal_id
                            )
                ...
                logger.info(
                    f"Completed processing folder {folder_id} - {folder_article_count} articles indexed"
                )

            logger.info(
                f"Completed processing {article_count} articles from {len(folder_ids)} folders"
            )
    ...
    def load_from_state(self) -> GenerateDocumentsOutput:
            ...
            if "freshdesk_folder_ids" in self.connector_specific_config:
                ...
                if isinstance(folder_ids_value, list):
                    ...
                    folder_ids.extend(
                        [
                            fid.strip()
                            for fid in folder_ids_value.split(",")
                            if fid.strip()
                        ]
                    )
                logger.info(
                    f"Using folder_ids from connector_specific_config['freshdesk_folder_ids']: {folder_ids}"
                )
        ...
        if hasattr(self, "folder_ids"):
            if isinstance(self.folder_ids, list):
                ...
                folder_ids.extend(self.folder_ids)
                logger.info(
                    f"Using folder_ids from self.folder_ids (list): {self.folder_ids}"
                )
            elif isinstance(self.folder_ids, str):
                # Multiple folder IDs provided as a comma-separated string
                parsed_ids = [
                    folder_id.strip()
                    for folder_id in self.folder_ids.split(",")
                    if folder_id.strip()
                ]
                folder_ids.extend(parsed_ids)
                logger.info(
                    f"Using folder_ids from self.folder_ids (string): parsed as {parsed_ids}"
                )

        if not folder_ids:
            raise ConnectorMissingCredentialError(
                "No Freshdesk KB folder_id(s) configured for load_from_state."
            )
        ...
        if not self.domain or not self.api_key:
            ...
            raise ConnectorMissingCredentialError(
                "Missing required Freshdesk credentials for indexing"
            )

        logger.info(
            f"Loading all solution articles from {len(folder_ids)} Freshdesk KB folders: {folder_ids}"
        )
    ...
    def poll_source(
        ...
        if (
            ...
            if "freshdesk_folder_ids" in self.connector_specific_config:
                ...
                logger.info(
                    f"Poll: Using folder_ids from connector_specific_config['freshdesk_folder_ids']: {folder_ids}"
                )
        ...
        if hasattr(self, "folder_ids"):
            if isinstance(self.folder_ids, list):
                ...
                folder_ids.extend(self.folder_ids)
                logger.info(
                    f"Poll: Using folder_ids from self.folder_ids (list): {self.folder_ids}"
                )
            elif isinstance(self.folder_ids, str):
                # Multiple folder IDs provided as a comma-separated string
                parsed_ids = [
                ...
                folder_ids.extend(parsed_ids)
                logger.info(
                    f"Poll: Using folder_ids from self.folder_ids (string): parsed as {parsed_ids}"
                )

        if not folder_ids:
            raise ConnectorMissingCredentialError(
                "No Freshdesk KB folder_id(s) configured for poll_source."
            )
        ...
        if not self.domain or not self.api_key:
            ...
            logger.error(f"Base URL: {self.base_url}, Auth: {bool(self.auth)}")
            raise ConnectorMissingCredentialError(
                "Missing required Freshdesk credentials for polling"
            )

        start_datetime = datetime.fromtimestamp(start, tz=timezone.utc)

        logger.info(
            f"Polling {len(folder_ids)} Freshdesk KB folders for updates since {start_datetime.isoformat()}"
        )
        logger.info(f"Using domain: {self.domain}, folders: {folder_ids}")
        yield from self._process_articles(folder_ids, start_datetime)

    def _get_slim_documents_for_article_batch(
        ...
        for article in articles:
            ...
            if article_id:
                ...
                slim_docs.append(
                    SlimDocument(id=_FRESHDESK_KB_ID_PREFIX + str(article_id))
                )
        return slim_docs
    ...
    def retrieve_all_slim_documents(
        ...
        if not folder_ids:
            raise ConnectorMissingCredentialError(
                "No Freshdesk KB folder_id(s) configured for slim document retrieval."
            )
        ...
        for folder_id in folder_ids:
            ...
            for article_batch in self._fetch_articles_from_folder(
                folder_id, start_datetime
            ):
                ...
                if len(slim_batch) >= self.batch_size:
                    logger.info(
                        f"Yielding batch of {len(slim_batch)} slim documents from folder {folder_id}"
                    )
                    yield slim_batch
                    slim_batch = []
            ...
            if slim_batch:
                logger.info(
                    f"Yielding final batch of {len(slim_batch)} slim documents from folder {folder_id}"
                )
                yield slim_batch

        logger.info(
            f"Completed retrieval of slim documents from {len(folder_ids)} folders"
        )
