...
from onyx.configs.app_configs import JSM_SLIM_PAGE_SIZE
...
from onyx.connectors.jsm.access import get_project_permissions
from onyx.connectors.jsm.utils import best_effort_basic_expert_info
from onyx.connectors.jsm.utils import best_effort_get_field_from_request
from onyx.connectors.jsm.utils import build_jsm_client
from onyx.connectors.jsm.utils import build_jsm_url
from onyx.connectors.jsm.utils import extract_text_from_adf
from onyx.connectors.jsm.utils import get_comment_strs
from onyx.connectors.jsm.utils import JSM_CLOUD_API_VERSION
...
_FIELD_REQUESTTYPE = "requesttype"
...
def _perform_jql_search(
    ...
    all_request_ids: list[list[str]] | None = None,
    checkpoint_callback: (
    ...
    a) this function returns an iterable of requests of length 0 < len(requests) <= max_results.
       - caveat; if all_request_ids is provided, the iterable will be the size of some sub-list.
       - this will only not match the above bound if a recent deployment changed max_results.

    IF the v3 API is used (i.e. the jsm instance is a cloud instance), then the caller should expect:
    ...
    b) this function will call checkpoint_callback ONCE after at least one of the following has happened:
       ...
       - a batch of requests has been bulk-fetched
    c) checkpoint_callback is called with the new all_request_ids and the pageToken of the enhanced
       search request. We pass in a pageToken of None once we've fetched all the request ids.
    ...
    if _is_cloud_client(jsm_client):
        ...
        return _perform_jql_search_v2(jsm_client, jql, start, max_results, fields)


def _handle_jsm_search_error(e: Exception, jql: str) -> None:
    """Handle common JSM search errors and raise appropriate exceptions.

    Args:
        e: The exception raised by the JSM API
    ...
    # Try to get status code and error text from JSMError or requests response
    ...
    if status_code == 400:
        if "does not exist for the field 'project'" in error_text:
            raise ConnectorValidationError(
                f"The specified JSM project does not exist or you don't have access to it. JQL query: {jql}. Error: {error_text}"
            )
        raise ConnectorValidationError(
...
def enhanced_search_ids(
    jsm_client: JSM, jql: str, nextPageToken: str | None = None
) -> tuple[list[str], str | None]:
    # https://community.atlassian.com/forums/JSM-articles/
    ...
    enhanced_search_path = jsm_client._get_url("search/jql")
    ...
    try:
        response = jsm_client._session.get(enhanced_search_path, params=params)
        response.raise_for_status()
        response_json = response.json()
    except Exception as e:
        _handle_jsm_search_error(e, jql)
        raise  # Explicitly re-raise for type checker, should never reach here

    return [str(request["id"]) for request in response_json["requests"]], response_json.get(
        "nextPageToken"
    )
...
def bulk_fetch_requests(
    ...
    # Prepare the payload according to JSM API v3 specification
    payload: dict[str, Any] = {"requestIdsOrKeys": request_ids}
...
def _perform_jql_search_v3(
    ...
    """
    The way this works is we get all the request ids and bulk fetch them in batches.
    However, for really large deployments we can't do these operations sequentially,
    as it might take several hours to fetch all the request ids.

    So, each run of this function does at least one of:
     - fetch a batch of request ids
     - bulk fetch a batch of requests

    If all_request_ids is not None, we use it to bulk fetch requests.
    """

    # with some careful synchronization these steps can be done in parallel,
    # leaving that out for now to avoid rate limit requests
    if not ids_done:
        new_ids, pageToken = enhanced_search_ids(jsm_client, jql, nextPageToken)
        if checkpoint_callback is not None:
            checkpoint_callback(chunked(new_ids, max_results), pageToken)

    # bulk fetch requests from ids. Note that the above callback MAY mutate all_request_ids,
    # but this fetch always just takes the last id batch.
    if all_request_ids:
        yield from bulk_fetch_requests(jsm_client, all_request_ids.pop(), fields)
...
def _perform_jql_search_v2(
    ...
    """
    Unfortunately, jsm server/data center will forever use the v2 APIs that are now deprecated.
    """
    logger.debug(
        f"Fetching JSM requests with JQL: {jql}, starting at {start}, max results: {max_results}"
    )
    try:
        requests = jsm_client.search_requests(
            jql_str=jql,
            startAt=start,
    ...
    for request in requests:
        if isinstance(request, Request):
            ...
            raise RuntimeError(f"Found JSM object not of type Request: {request}")


def process_jsm_request(
    jsm_base_url: str,
    request: Request,
    ...
    if labels_to_skip:
        if any(label in request.fields.labels for label in labels_to_skip):
            logger.info(
                f"Skipping {request.key} because it has a label to skip. Found "
                f"labels: {request.fields.labels}. Labels to skip: {labels_to_skip}."
            )
            return None

    if isinstance(request.fields.description, str):
        description = request.fields.description
    else:
        description = extract_text_from_adf(request.raw["fields"]["description"])
    ...
    # Check ticket size
    if len(ticket_content.encode("utf-8")) > JSM_CONNECTOR_MAX_TICKET_SIZE:
        logger.info(
            f"Skipping {request.key} because it exceeds the maximum size of {JSM_CONNECTOR_MAX_TICKET_SIZE} bytes."
        )
        return None

    page_url = build_jsm_url(jsm_base_url, request.key)

    metadata_dict: dict[str, str | list[str]] = {}
    people = set()

    creator = best_effort_get_field_from_request(request, _FIELD_REPORTER)
    ...
    assignee = best_effort_get_field_from_request(request, _FIELD_ASSIGNEE)
    ...
    metadata_dict[_FIELD_KEY] = request.key
    if priority := best_effort_get_field_from_request(request, _FIELD_PRIORITY):
        metadata_dict[_FIELD_PRIORITY] = priority.name
    if status := best_effort_get_field_from_request(request, _FIELD_STATUS):
        metadata_dict[_FIELD_STATUS] = status.name
    if resolution := best_effort_get_field_from_request(request, _FIELD_RESOLUTION):
        metadata_dict[_FIELD_RESOLUTION] = resolution.name
    if labels := best_effort_get_field_from_request(request, _FIELD_LABELS):
        metadata_dict[_FIELD_LABELS] = labels
    if created := best_effort_get_field_from_request(request, _FIELD_CREATED):
        metadata_dict[_FIELD_CREATED] = created
    if updated := best_effort_get_field_from_request(request, _FIELD_UPDATED):
        metadata_dict[_FIELD_UPDATED] = updated
    if duedate := best_effort_get_field_from_request(request, _FIELD_DUEDATE):
        metadata_dict[_FIELD_DUEDATE] = duedate
    if requesttype := best_effort_get_field_from_request(request, _FIELD_REQUESTTYPE):
        metadata_dict[_FIELD_REQUESTTYPE] = requesttype.name
    if resolutiondate := best_effort_get_field_from_request(
        request, _FIELD_RESOLUTION_DATE
    ):
        metadata_dict[_FIELD_RESOLUTION_DATE_KEY] = resolutiondate

    parent = best_effort_get_field_from_request(request, _FIELD_PARENT)
    if parent is not None:
        metadata_dict[_FIELD_PARENT] = parent.key

    project = best_effort_get_field_from_request(request, _FIELD_PROJECT)
    ...
    return Document(
        ...
        semantic_identifier=f"{request.key}: {request.fields.summary}",
        title=f"{request.key} {request.fields.summary}",
        doc_updated_at=time_str_to_utc(request.fields.updated),
...
class JSMConnector(
    ...
    def __init__(
        ...
        # Custom JQL query to filter JSM requests
        ...
        # when making jsm requests but still want correct links to requests in the UI.
        # So, the user's base url is stored here, but converted to a scoped url when passed
        # to the jsm client.
        self.jsm_base = jsm_base_url.rstrip("/")  # Remove trailing slash if present
        self.jsm_project = project_key
        self._comment_email_blacklist = comment_email_blacklist or []
        self.labels_to_skip = set(labels_to_skip)
        self.jql_query = jql_query
        self.scoped_token = scoped_token
        self._jsm_client: JSM | None = None
    ...
    @property
    def jsm_client(self) -> JSM:
        if self._jsm_client is None:
            raise ConnectorMissingCredentialError("JSM")
        return self._jsm_client

    @property
    def quoted_jsm_project(self) -> str:
        # Quote the project name to handle reserved words
        if not self.jsm_project:
            return ""
        return f'"{self.jsm_project}"'
    ...
    def _get_project_permissions(
        ...
        Args:
            project_key: The JSM project key
        ...
        if cache_key not in self._project_permissions_cache:
            self._project_permissions_cache[cache_key] = get_project_permissions(
                jsm_client=self.jsm_client,
                jsm_project=project_key,
                add_prefix=add_prefix,
            )
        return self._project_permissions_cache[cache_key]

    def _is_epic(self, request: Request) -> bool:
        """Check if request is an Epic."""
        requesttype = best_effort_get_field_from_request(request, _FIELD_REQUESTTYPE)
        if requesttype is None:
            return False
        return requesttype.name.lower() == "epic"

    def _is_parent_epic(self, parent: Any) -> bool:
        """Check if a parent reference is an Epic.

        The parent object from request.fields.parent has a different structure
        than a full Request, so we handle it separately.
        """
        parent_requesttype = (
            getattr(parent.fields, "requesttype", None)
            if hasattr(parent, "fields")
            else None
        )
        if parent_requesttype is None:
            return False
        return parent_requesttype.name.lower() == "epic"
    ...
    def _yield_project_hierarchy_node(
        ...
        yield HierarchyNode(
            ...
            link=f"{self.jsm_base}/projects/{project_key}",
    ...
    def _yield_epic_hierarchy_node(
        ...
        """Yield a hierarchy node for an Epic request."""
        request_key = request.key
        if request_key in seen_hierarchy_node_ids:
            return

        seen_hierarchy_node_ids.add(request_key)

        yield HierarchyNode(
            raw_node_id=request_key,
            raw_parent_id=project_key,
            display_name=f"{request_key}: {request.fields.summary}",
            link=build_jsm_url(self.jsm_base, request_key),
    ...
    def _yield_parent_hierarchy_node_if_epic(
        ...
        """Yield hierarchy node for parent request if it's an Epic we haven't seen."""
        ...
        yield HierarchyNode(
            ...
            link=build_jsm_url(self.jsm_base, parent_key),
            node_type=HierarchyNodeType.FOLDER,  # don't have a separate epic node type
        )

    def _get_parent_hierarchy_raw_node_id(self, request: Request, project_key: str) -> str:
        """Determine the parent hierarchy node ID for an request.

        Returns:
            - Epic key if request's parent is an Epic
            - Project key otherwise (for top-level requests or non-epic parents)
        """
        parent = best_effort_get_field_from_request(request, _FIELD_PARENT)
        if parent is None:
    ...
    def load_credentials(self, credentials: dict[str, Any]) -> dict[str, Any] | None:
        self._jsm_client = build_jsm_client(
    ...
    def _get_jql_query(
        ...
        if self.jsm_project:
            base_jql = f"project = {self.quoted_jsm_project}"
    ...
    def _load_from_checkpoint(
        ...
        for request in _perform_jql_search(
            jsm_client=self.jsm_client,
            jql=jql,
            start=current_offset,
            max_results=_JSM_FULL_PAGE_SIZE,
            all_request_ids=new_checkpoint.all_request_ids,
            checkpoint_callback=checkpoint_callback,
            ...
            try:
                ...
                if project_key:
                    ...
                    # 3. If this request IS an Epic, yield it as hierarchy node
                    if self._is_epic(request):
                        yield from self._yield_epic_hierarchy_node(
                            request, project_key, seen_hierarchy_node_ids
                        )

                # Determine parent hierarchy node ID for the document
                parent_hierarchy_raw_node_id = (
                    self._get_parent_hierarchy_raw_node_id(request, project_key)
                    if project_key
                    else None
                )

                if document := process_jsm_request(
                    jsm_base_url=self.jsm_base,
                    request=request,
                ...
                yield ConnectorFailure(
                    failed_document=DocumentFailure(
                        ...
                        document_link=build_jsm_url(self.jsm_base, request_key),
                    ),
    ...
    def update_checkpoint_for_next_run(
        ...
        if _is_cloud_client(self.jsm_client):
            # other updates done in the checkpoint callback
            checkpoint.has_more = (
                len(checkpoint.all_request_ids) > 0 or not checkpoint.ids_done
            )
        else:
    ...
    def retrieve_all_slim_docs_perm_sync(
        ...
        )  # we add one day to account for any potential timezone requests
        ...
        while checkpoint.has_more:
            for request in _perform_jql_search(
                ...
                jql=jql,
                start=current_offset,
                max_results=JSM_SLIM_PAGE_SIZE,
                all_request_ids=checkpoint.all_request_ids,
                checkpoint_callback=checkpoint_callback,
                ...
                request_key = best_effort_get_field_from_request(request, _FIELD_KEY)
                doc_id = build_jsm_url(self.jsm_base, request_key)
                ...
                current_offset += 1
                if len(slim_doc_batch) >= JSM_SLIM_PAGE_SIZE:
                    yield slim_doc_batch
                    slim_doc_batch = []
            self.update_checkpoint_for_next_run(
                checkpoint, current_offset, prev_offset, JSM_SLIM_PAGE_SIZE
            )
    ...
    def validate_connector_settings(self) -> None:
        ...
        elif self.jsm_project:
            try:
                self.jsm_client.project(self.jsm_project)
            except Exception as e:
                self._handle_jsm_connector_settings_error(e)
        else:
            # If neither JQL nor project specified, validate we can access the JSM API
            try:
                # Try to list projects to validate access
                self.jsm_client.projects()
            except Exception as e:
                self._handle_jsm_connector_settings_error(e)

    def _handle_jsm_connector_settings_error(self, e: Exception) -> None:
        """Helper method to handle JSM API errors consistently.

        Extracts error messages from the JSM API response for all status codes when possible,
        ...
        if status_code == 401:
            raise CredentialExpiredError(
                "JSM credential appears to be expired or invalid (HTTP 401)."
            )
        elif status_code == 403:
            raise InsufficientPermissionsError(
                "Your JSM token does not have sufficient permissions for this configuration (HTTP 403)."
            )
        elif status_code == 429:
            raise ConnectorValidationError(
                "Validation failed due to JSM rate-limits being exceeded. Please try again later."
            )
...
def make_checkpoint_callback(
    ...
    def checkpoint_callback(
        request_ids: Iterator[list[str]], pageToken: str | None
    ) -> None:
        for id_batch in request_ids:
            checkpoint.all_request_ids.append(id_batch)
        checkpoint.cursor = pageToken
        # pageToken starts out as None and is only None once we've fetched all the request ids
