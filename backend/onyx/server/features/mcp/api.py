...
from onyx.server.features.mcp.models import MCPOAuthCallbackRequest
...
def _initiate_oauth(
    ...
    redirect_uri = f"{WEB_DOMAIN}/api/mcp/oauth/callback"
...
def oauth_callback(
    request: MCPOAuthCallbackRequest = Depends(),
    ...
    web_redirect_uri = f"{WEB_DOMAIN}{state_data.return_path}"
    redirect_uri = f"{WEB_DOMAIN}/api/mcp/oauth/callback"
    ...
    logger.info(
        ...
        f"redirect_url={web_redirect_uri}"
    )

    # TODO: unknown why this doesn't work
    # return RedirectResponse(web_redirect_uri)

    next_url = f"{WEB_DOMAIN}{state_data.return_path}"
    body = f"""<!doctype html><meta charset="utf-8">
    <title>Redirecting…</title>
    <p>Redirecting…</p>
    <script>location.replace({json.dumps(next_url)});</script>"""
...
def _add_tools_to_server(
    ...
    auth_performer = get_mcp_server_auth_performer(mcp_server)
    if mcp_server.admin_connection_config is None:
        connection_headers = {}
    else:
        # Get headers for discovery
        connection_headers = mcp_server.admin_connection_config.config.get(
            "headers", {}
        )
        if auth_performer == MCPAuthenticationPerformer.PER_USER:
            connection_headers = _build_headers_from_template(
                MCPAuthTemplate(
                    headers=connection_headers,
                    required_fields=list(connection_headers.keys()),
                ),
                mcp_server.admin_connection_config.config.get(
                    "header_substitutions", {}
                ),
    ...
    available_tools = discover_mcp_tools(
        mcp_server.server_url, connection_headers, transport=transport
    )
    ...
    for tool_name in selected_tools:
        ...
        tool.display_name = tool_def.get("displayName", tool_name)
        tool.mcp_input_schema = tool_def.get("inputSchema") or tool_def.get(
            "input_schema"
        )
...
def list_mcp_tools_by_id(
    ...
    if mcp_server.auth_type == MCPAuthenticationType.API_TOKEN.value:
        if (
            get_mcp_server_auth_performer(mcp_server)
            == MCPAuthenticationPerformer.PER_USER
        ):
            # Per-user auth only: require the user's config
            user_email = user.email if user else ""
            connection_config = get_user_connection_config(
                server_id=server_id, user_email=user_email, db_session=db
            )
            if not connection_config:
                raise HTTPException(
                    status_code=401,
                    detail="User authentication required for this MCP server",
                )
        else:
            # No per-user template: use admin config
            connection_config = mcp_server.admin_connection_config
    elif mcp_server.auth_type == MCPAuthenticationType.OAUTH.value:
        # OAuth in admin panel: use admin config (same as API token admin auth)
        connection_config = mcp_server.admin_connection_config
        if not connection_config:
            raise HTTPException(
                status_code=401,
                detail="OAuth authentication required for this MCP server",
            )
    ...
    return {
        ...
        "tools": tools,
        # "resources": resources,  # TODO: Include resources when needed
    }
