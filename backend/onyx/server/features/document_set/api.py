...
def list_document_sets_for_user(
    # Filter vocabulary. READ_DOCUMENT_SETS is deliberately not used: it means
    # "see every document set", which this route must not grant — the results
    # stay ACL-filtered below.
    user: User = Depends(require_permission(Permission.READ_SEARCH_FILTERS)),
    db_session: Session = Depends(get_session),
