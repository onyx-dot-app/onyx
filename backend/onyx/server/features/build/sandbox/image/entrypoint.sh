...
OPENCODE_PORT=4096
# Bind host for opencode serve. Defaults to 0.0.0.0 (IPv4). On IPv6-only
# clusters set OPENCODE_SERVE_HOSTNAME=:: so the api-server can reach it over
# IPv6 (a "::" socket also accepts IPv4-mapped clients on Linux).
