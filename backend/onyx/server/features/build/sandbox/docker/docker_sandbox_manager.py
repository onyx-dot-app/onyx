...
class DockerSandboxManager(SandboxManager):
    ...
    def get_image_state(self) -> SandboxImageState:
        ...
        Every sandbox is movable: they share the one host the target was resolved
        from, so there is no per-node coverage question as there is on Kubernetes.
        """
        return SandboxImageState(
            target=self._current_image_target(),
            movable_digests=self._live_sandbox_digests(),
        )
