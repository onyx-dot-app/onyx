...
@router.post("")
def create_api_key(
    create_args: CreateAPIKeyArgs,
    user: User | None = Depends(current_admin_user),
    db_session: Session = Depends(get_session),
) -> ApiKeyDescriptor:
    return insert_api_key(db_session, create_args, user.id if user else None)
...
def update_existing_api_key(
    api_key_id: int,
    update_args: UpdateAPIKeyArgs,
    _: User | None = Depends(current_admin_user),
    db_session: Session = Depends(get_session),
) -> ApiKeyDescriptor:
    return update_api_key(db_session, api_key_id, update_args)
