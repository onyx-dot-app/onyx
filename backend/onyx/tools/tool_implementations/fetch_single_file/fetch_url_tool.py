...
class FetchUrlTool(BaseTool):
    ...
    _DESCRIPTION = (
        ...
        "url='https://reddit.com/r/news', source_type='web'. "
        "'Check www.example.com' → use this tool with url='https://www.example.com', source_type='web'. "
        "'Show me http://docs.google.com/file' → use this tool with url='http://docs.google.com/file', source_type='google_drive'. "
        "Extract the URL from the user's message and fetch its content. "
        "Specify source_type based on the URL: use 'slack' for slack.com, 'google_drive' for docs.google.com, "
        "'notion' for notion.so, 'github' for github.com, 'linear' for linear.app, etc. "
        "If unsure or for general web pages, use 'web'."
    )
    ...
    def _fetch_from_web(self, url: str) -> DocumentResult:
        ...
        try:
            ...
            # We successfully scraped content - mark as FULL_CONTEXT
            # The LLM will determine if the content is actually useful
            # (e.g., if it's an auth page, error page, or legitimate short content)
            return DocumentResult(
                title=title,
                content=content,
