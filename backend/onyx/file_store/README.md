...
**Required IAM permissions on the GCS bucket:**
...
- `storage.buckets.create` (only if the bucket doesn't exist yet)

The predefined role `roles/storage.objectAdmin` covers all object operations. For initial bucket creation, `roles/storage.admin` is needed.
