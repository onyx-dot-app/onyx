...
You are a helpful assistant that can analyze and think through the steps required to  gather the information required to (later) \
...
Here are some principle reminders about how to think about gathering the information required to \
(later) answer the user's question:
   - you will derive the steps through a conversation with the user.
   - the information will be acquired through tool calls that you will orchestrate.
...
# So DO NOT extract information pertaining to other companies, other users, or other topics that \
...
Note:
  - for "user_background", "company_background", "answer_preferences" and "internal_search_topics" updates, consider \
...
add a new key.
  ...
  - for updates to "internal_search_topics", consider intial information and the new information \
("covered" implies that a topc is covered in the internal documents, "not_covered" implies that a \
topic is not well-covered in the internal documents). Compare tyhe topics and decide whether \
updates should be made. If a topic was covered initially, but based on the new information, it \
appears to be not well-covered, keep it as "covered". So if in doubt, keep "covered" intact.
...
"internal_search_topics": <return here a dictionary with keys "covered" or "not_covered" and values being lists of topics>,
}}
"""


CS_COMPRESSION_PROMPT_TEMPLATE = """
Here is a dictionary with the current memory information:

---
---original_cheatsheet---
---

Please create a consolidated memory information dictionary in the following manner:
   - the top-level keys must be the same as in the original dictionary (user_background, company_background, answer_preferences, \
internal_search_topics). Never mix information from top-level keys with different keys into each other. The corresponding \
information is fully independent and separate.
   - consolidate as much as possible, but without changing the meaning of the information.
   - also, do NEVER change any "name" sub-keys
   - the new dictionary should ideally have not more than about 1500 words
   - when you consolidate information for a top-level key, consider its purpose (see below) and the information itself.
   - use double quotes for all keys and strings.

Top-level key-specific instructions:
   - user_background:
    - purpose: to provide context about the user who asks the question to inform future task approaches.
    - create no more than 5 sub-keys. (If there are less than 5 sub-keys, keep the sub-keys as is.)
    - DO NOT infer the users role or interest from questions asked or other information in the \
memory dictionary. Only look at information explicitly stated in the memory dictionary for key "user_background".
    - each sub-key should have no more than 5 strings
   - company_background:
    - purpose: to provide context about the company the user is working at to inform future task approaches.
    - create no more than 5 sub-keys. (If there are less than 5 sub-keys, keep the sub-keys as is.)
    - each sub-key should have no more than 5 strings
   - answer_preferences:
    - purpose: to provide context about the user's preferred answer style to inform future answer generations.
    - stay with these sub-keys: style, length, comparison_format
    - consolidate the information into a single string for each sub-key
   - internal_search_topics:
    - purpose: to provide context about the topics that are well-represented in the internal corpus \
to inform future search decisions.
    - stay with these sub-keys: covered, not_covered
    - try to consolidate the 'covered' topics into no more than 10 short topics that represent the \
original topics well.
    - consider the current topics under 'not_covered' and see whether any of them are actually somewhat \
represented under 'covered'. If so, drop it from 'not_covered'. Consolidate the remaining 'not_covered' topics into \
no more than 5 short topics.
...
You are an expert in analyzing the history of search questions and answers to identify what types of topics \
are well-represented in in the internal corpus vs those that are not.

You will receice from the user a history of search questions and answers, and your task is to identify whether \
there are topics you think are well-represented in in the internal corpus vs those that are not.

Hints:
  - keep topics quite high-level as in - for example - 'products', 'animals', 'cars', etc.
  - only include a topic as NOT-well-represented in the internal corpus if the question was on \
the level of the topic! I.e., if a question about 'product X' did not return and results, do NOT assume \
that 'products' are not represented in the corpus. It really needs a more general question about 'products'. \
(And also do not include 'product X' as not-well-represented as it is not on the correct level.)
  - on the other hand, if you see information about 'product X', you CAN assume that 'products' \
are well-represented in the corpus.

Please respond with a json dictionary in this format:
{{
  "covered": <return here a list of high-level topics that, based on the observations, that you think ARE \
well-represented in in the internal corpus >,
  "not_covered": <return here a list of high-level topics that, based on the observations, that you think \
ARE NOT well-represented in in the internal corpus >,
}}
"""
