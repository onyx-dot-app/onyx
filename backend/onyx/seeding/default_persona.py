...
class DefaultPersona(BaseModel):
    ...
    # Task prompt (always updated to ensure latest system behavior)
    ...
    # System prompt (admin can customize instructions)
    system_prompt: str = Field(..., description="System prompt - admin controlled")
...
def apply_always_updated_fields(
    ...
    fields (e.g. system_prompt, retrieval, UI, and overrides) are left unchanged.
    """
    existing_persona.name = default_persona.name
    existing_persona.description = default_persona.description
    # system_prompt is now admin-controlled, not updated on restart
