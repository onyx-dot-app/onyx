...
A token scoped to `read:search` covers the document-search tool: it implies `read:search_filters`,
which is what lists the indexed sources and document sets a search may be filtered by. Add
