...
Use one query per `internal_search` call. When a question covers distinct subjects that need their own evidence, make a separate `internal_search` call for each - they run in parallel. Never provide more than 3 queries at once to `internal_search`.
""".lstrip()
