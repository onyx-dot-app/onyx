...
- Glomi Forge 子项目 A 收尾：完成 Task 13-16。新增 `/api/glomi-forge` FastAPI router 与 SSE stream helper，提供 create/get/terminate/instruction/events 端点，并注册到全局 `/api` 前缀；feature flag 关闭时通过 `OnyxError(NOT_FOUND)` 保护，路由未使用 `response_model`。新增轻量 route-level API 测试，避免真实 LLM/Celery/DB 依赖。
- Glomi Forge 最小内测前端：新增 `web/src/hooks/useGlomiForgeSession.ts` SWR polling hook、`fetchGlomiForgeSession` helper 与 `web/src/app/glomi-forge-dev/page.tsx`。页面支持中文需求输入、创建 landing_page session、轮询状态、展示 preview iframe 和错误状态。按前端规范使用 Opal Button、`@/` 绝对导入和自定义 Tailwind token；新增 hook Jest 单测覆盖成功解析与失败抛错。
- Glomi Forge gated E2E：新增 `backend/tests/external_dependency_unit/glomi_forge/test_e2e_landing_page.py`，只有同时配置 `DAYTONA_API_URL` 和 `DAYTONA_API_KEY` 时才用真实 `DaytonaSandboxProvider` + `PiBuilderAdapter` + `ForgeOrchestrator` 跑落地页端到端；当前本机 `.vscode/.env` 未提供完整 Daytona env，验证结果为 `1 skipped`。
- 类型/验证补强：`DaytonaSandboxProvider` 改为 `importlib.import_module("daytona")` 延迟加载 SDK，避免本机 `.venv` 未安装 Daytona 时 `ty` unresolved import；`ForgeSpecBuilder` 显式标注 LLM messages 为 `list[ChatCompletionMessage]`。前端 `GlomiLogotype` 不再读取被声明为 string 的 PNG import `.width/.height`，改用实际 wordmark 比例常量，解除 `npm run types:check` 阻塞。
- 验证记录：`ENABLE_GLOMI_FORGE=true pytest backend/tests/external_dependency_unit/glomi_forge/test_api.py -v` 通过 `1 passed`；`python -m dotenv -f .vscode/.env run -- pytest backend/tests/external_dependency_unit/glomi_forge/test_e2e_landing_page.py -v` 为 `1 skipped`；`ty check backend/onyx/glomi_forge/` 通过；`pytest backend/tests/unit/glomi_forge/ -v` 通过 `13 passed`；`cd web && npm test -- src/hooks/useGlomiForgeSession.test.ts --runInBand` 通过 `2 passed`；`cd web && npm run types:check && npm run lint` 通过；`ruff check` 与 `git diff --check` 针对 touched 文件通过。
- 当前环境阻塞：计划里的 `uv run ty check ...` / `uv run pre-commit ...` 仍会先触发 `torch==2.9.1` 在 macOS x86_64 无 wheel 的 sync/resolve 问题；改用已存在 `.venv` 直接跑 `ty`/pytest 取得验证证据。`pre-commit run --all-files` 额外在安装 Go hook 依赖时访问 `proxy.golang.org` 超时，未进入完整 hook 阶段。DB-backed external dependency tests 和 Alembic 实跑仍受本机 Postgres/Docker 不可用限制；真实 Daytona/Pi E2E 等 INFRA 提供 control plane 与 snapshot。
- Glomi Forge Task 12 继续推进：新增 `glomi_forge.run_session` Celery shared task、`enqueue_forge_session(..., expires=3600)` helper、`OnyxCeleryTask.GLOMI_FORGE_RUN_SESSION` 常量，并把 `onyx.background.celery.tasks.glomi_forge` 加入 primary worker autodiscovery。任务内部组装 `DaytonaSandboxProvider` + `PiBuilderAdapter` 并调用 `ForgeOrchestrator`，同时暴露 `_run` 便于 fake provider/adapter 测试。验证记录：Ruff、py_compile、`git diff --check` 通过；external dependency test 已进入 DB 创建 session 后因本机 Postgres `127.0.0.1:5432` connection refused 阻塞。
...
- Glomi Forge Task 7 继续推进：新增 `ForgeOrchestrator` 状态机，负责从 DB session 读取 `ForgeSpec`、解析模板、调用 `SandboxProvider` 创建沙箱并写入 task/context 文件、调用 `BuilderAdapter` 启动构建、消费并持久化 `ForgeEvent`，对 `PreviewReady` / `ArtifactReady` / `BuilderFinished` / `BuilderFailed` 分别更新 preview、output、completed 或 failed，并在失败路径清理 sandbox。新增 external dependency test `test_orchestrator.py`；红测先确认 orchestrator 模块缺失，implementation 后 Ruff/py_compile 通过。实际测试仍因本机 `127.0.0.1:5432` Postgres connection refused 卡在 `db_session` fixture，与 Task 3/4 同一环境阻塞。
- Glomi Forge Task 6 继续推进：新增 Forge 相关 `OnyxErrorCode`、`LLMFlow.FORGE_SPEC_GENERATION`，以及 `TemplateService` / `TemplateDescriptor` 和 landing_page 模板资产（`AGENTS.md`、`SYSTEM.md`、`output_contract.md`）。模板服务从 `configs.py` 读取默认 snapshot 与 preview port，当前只支持 `ForgeArtifactType.LANDING_PAGE`。新增 `backend/tests/unit/glomi_forge/test_template_service.py`，红测先确认 service package 缺失；实现后通过 `2 passed`，touched Python Ruff 与 `git diff --check` 通过。
- Glomi Forge Task 5 继续推进：新增 `SandboxProvider` / `BuilderAdapter` Protocol 接口和 `FakeSandboxProvider` / `FakeBuilderAdapter` 测试替身，fakes 会记录 create/read/write/command/preview/subscribe/stop 调用，方便后续 orchestrator 单测断言交互。新增 `backend/tests/unit/glomi_forge/test_fakes.py`，红测先确认 testing package 缺失；实现后通过 `2 passed`，touched interfaces/testing/test Ruff 与 `git diff --check` 通过。
- Glomi Forge Task 4 继续推进：新增 `backend/onyx/db/glomi_forge.py`，集中承载 Glomi Forge session/event 的 DB 操作（create/get/status/sandbox/builder/preview/output/failed/append/fetch），并新增 `backend/tests/external_dependency_unit/glomi_forge/test_db.py` 覆盖创建状态更新与事件 seq 单调递增。红测先确认 `onyx.db.glomi_forge` 缺失；实现后 Ruff、`py_compile`、`git diff --check` 通过。外部依赖测试当前无法完成：测试已进入 DB 操作后因 `127.0.0.1:5432` Postgres connection refused 失败，与 Task 3 迁移应用同一环境阻塞。
- Glomi Forge Task 3 继续推进：新增 `GlomiForgeStatus` / `ForgeArtifactType` 枚举、`GlomiForgeSession` / `GlomiForgeEvent` ORM 表，以及 Alembic revision `487a16fc2925_add_glomi_forge_tables.py`。ORM 对新 Glomi Forge 枚举显式使用 enum value 存储，避免 SQLAlchemy `Enum(native_enum=False)` 默认写 enum name 与迁移里的小写字符串值不一致。验证记录：模型契约测试 `backend/tests/external_dependency_unit/glomi_forge/test_models.py` 通过 `2 passed`，touched DB/migration/test Ruff 通过，迁移文件 `py_compile` 通过，Alembic head 更新为 `487a16fc2925`。本机无法实际 `alembic upgrade head`：`.vscode/.env` 指向 `POSTGRES_HOST=localhost`，但 `127.0.0.1:5432` 拒绝连接，Docker daemon 也不可用，因此迁移应用需在可达 Postgres 的环境补跑。
...
- MiniMax 多模态适配补强：后端模型目录把 `MiniMax-M3` 以及通过 `GLOMI_MINIMAX_LLM_MODEL_NAMES` 增加的 MiniMax 模型都按支持图片输入处理，并带 `vision` 角色；`/api/chat/available-models` 透传 `supports_image_input`，前端 selector/popover 保留为 `supportsImageInput`，用于图片上传门控与“图片”能力展示。
